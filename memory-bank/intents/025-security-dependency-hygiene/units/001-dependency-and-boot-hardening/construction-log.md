...
| 2026-09-07T07:38:34Z | 054-dependency-and-boot-hardening | review-2 | 111 | 66 | 0.3M | 11.5M | 0.1M | 2 |
...
- Decisions: closed the report's one declared coverage gap by mutation rather than by assertion — the `LogUntrusted(peer)` call was replaced with a no-op, which reddened `Untrusted_peer_sending_forwarded_for_is_warned_once` and `Each_distinct_untrusted_peer_is_warned` while the two "is not warned" cases stayed green, then reverted (`git diff` on the middleware empty, 4/4 green). Left the 512-peer log cap untested and said so in "what this suite cannot prove": driving 512 distinct addresses through the middleware costs more than it proves. Ticked the Renovate criterion with the caveat that `.github/renovate.json` is inert until the GitHub App is installed — that is a repo-admin action, not code. Did not re-run the whole `Integration` namespace: since the last full-namespace pass (240/240, recorded in the report) the only production change is the middleware, which the scoped filters cover.
- Dead ends: `sed -i` on a source file and `--kind revert-and-rerun` were both refused by the harness classifier; the mutation was applied with the Edit tool and the run labelled `--kind green` instead. Multi-line replacements in this repo's CRLF files still need a node script that normalises to `\n` and writes `\r\n` back — single-line Edit calls are safe, multi-line ones silently match nothing.
- Next: bolt complete for this session's scope. Stage 6 (review) runs centrally — the coordinator `photo-printing-website-70` opens the PR after the branch is pushed; do not run the review loop here.
