...
The complete backend payment integration: Stripe (PaymentIntent, webhook) and the legacy processor (redirect initiate, IPN callback). Both processors share the `OrderService` which creates Orders from carts and fires post-payment side effects (email trigger, upload association).
...
- the legacy processor: HMAC-MD5 field order is exact per specification — any ordering mistake breaks all payments
...
| 004 | legacy-processor-initiate | FR-15 | 016 |
| 005 | legacy-processor-ipn-handler | FR-15 | 016 |
