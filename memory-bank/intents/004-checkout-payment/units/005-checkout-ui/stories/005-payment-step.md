...
**As a** customer
**I want** to pay with either an international card (Stripe) or a Romanian card (the legacy processor)
**So that** I can complete my purchase using whichever payment method works for my bank

## Acceptance Criteria

- [ ] **Given** Step 3 (`/checkout/plata`) is loaded, **When** rendered, **Then** two tabs are shown: `Card internațional (Stripe)` and `Card românesc (the legacy processor)`
...
- [ ] **Given** the legacy processor tab is active and `Plătește cu the legacy processor` is clicked, **When** `POST /api/payments/legacy-processor/initiate` responds, **Then** `window.location.href` is set to the `redirectUrl`
...
- the legacy processor tab: no form — just a button and logo; call `POST /api/payments/legacy-processor/initiate` on click
- On `confirmCardPayment` success: route to `/comanda/{orderId}/confirmare?processor=stripe`
- the legacy processor return URL (configured on the legacy processor side): `/comanda/{orderId}/confirmare?processor=legacy-processor`

## Dependencies

### Requires
- Story 001-checkout-stepper (CheckoutStateService)
- Bolt 016 (payment-backends — `POST /api/payments/stripe/intent` + `POST /api/payments/legacy-processor/initiate`)
...
| User switches tabs (Stripe → the legacy processor) | the legacy processor initiation creates a NEW order (Stripe order abandoned in AwaitingPayment) |
| Stripe 3DS challenge appears | Stripe.js handles 3DS natively in the embedded element |
| the legacy processor redirect fails (network) | Browser shows network error; user returns via back button to checkout |
...
- Split payment (partial Stripe + partial the legacy processor)
