...
- [ ] **Given** `?processor=euplatesc` is in the URL, **When** the page loads, **Then** the page polls `GET /api/orders/{orderId}` up to 5 times with 2s interval until status is `Paid` (IPN may not have arrived yet)
...
- Polling for EuPlatesc: `?processor=euplatesc` triggers `interval(2000).pipe(take(5), switchMap(() => ordersService.getById(orderId)), filter(o => o.status === 'Paid'), take(1))`
...
|----------|-------------------|
| EuPlatesc IPN delayed > 10s | Polling ends; show `"Plata este în curs de procesare"` with manual refresh hint |
