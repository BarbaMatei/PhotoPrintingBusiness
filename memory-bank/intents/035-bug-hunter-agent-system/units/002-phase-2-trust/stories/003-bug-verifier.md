...
- [ ] **Given** Prompt 10, **When** built, **Then** skill `bug-verifier` exists (agent-as-skill), created via skill-creator, and the brief's three test prompts pass (null-deref confirmed via failing test run twice; guarded candidate disproven & dropped; unbuildable sandbox → "could not verify" + environment flagged) *(2026-09: a component that extends the review loop is a script or skill edit under `reviews/lib` / `.claude/skills`, built and tested there; skill-creator applies only to a new standalone skill — FR-1 as amended)*
