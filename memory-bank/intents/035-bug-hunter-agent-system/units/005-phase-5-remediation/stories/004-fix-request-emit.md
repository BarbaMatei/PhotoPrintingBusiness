...
- [ ] **Given** Prompt 33, **When** built, **Then** skill `fix-request-emit` exists, created via skill-creator, and the brief's three test prompts pass (fresh correlation_id emission; re-run updates not duplicates; never calls AI-DLC or edits source) *(2026-09: a component that extends the review loop is a script or skill edit under `reviews/lib` / `.claude/skills`, built and tested there; skill-creator applies only to a new standalone skill — FR-1 as amended)*
