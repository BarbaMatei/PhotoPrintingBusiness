---
id: 003-legacy-processor-initiate-idempotency
...
# Story: 003-legacy-processor-initiate-idempotency

## User Story

**As** a checkout user paying via the legacy processor
**I want** clicking "Pay" twice to redirect me to the same the legacy processor page
**So that** I don't end up with two orders awaiting payment

## Acceptance Criteria

- [ ] **Given** two `POST /api/payments/legacy-processor/initiate` calls with the same `Idempotency-Key`, **Then** the response body returns the same redirect URL and the same `OrderId`.
...
- the legacy processor has no server-side idempotency primitive. The only guarantee is that we never persist two `Orders` for the same `Idempotency-Key`.
...
- Persist the constructed redirect URL on `Order` (new nullable `LegacyProcessorRedirectUrl` column may be desirable — decide in bolt plan; otherwise reconstruct on the fly from the persisted order).
...
|----------|-------------------|
| Customer returns from the legacy processor cancellation flow with the same key | Reuse the existing order + URL |
