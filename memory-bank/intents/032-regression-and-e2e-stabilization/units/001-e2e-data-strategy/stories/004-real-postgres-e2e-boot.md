...
**As a** maintainer who knows the in-memory/SQLite test gap exists
...
- [ ] **Given** the e2e harness, **When** it boots, **Then** the API runs with `DatabaseProvider=Postgres` against the compose Postgres (reusing `docker-compose.yml`'s db service), not SQLite or in-memory
...
- [ ] **Given** a migration that only works on SQLite, **When** it is applied against the compose Postgres, **Then** the e2e boot fails loudly (surfacing the DEPLOYMENT.md §7 gap) rather than silently passing
...
- This story deliberately does NOT fix the SQLite-vs-Postgres migration (that is migration-guard CI work) — it makes the e2e suite the place the gap surfaces.
