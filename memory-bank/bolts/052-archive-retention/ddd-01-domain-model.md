...
8. **Postgres-typed migration if we add `CompletedAt`.** Same gotcha as bolt 051 — the PostgreSQL scaffold generates `INTEGER`, needs hand-edit to `timestamp with time zone` for Postgres. Stage 4 concern; flagged here so it isn't missed.
