...
- **`SetBeforeSend` runs on a worker thread.** The scrubber must be allocation-light and never throw — a throw drops the event silently.
- **Adding a sensitive header**: append to `SensitiveHeaders` in `SentryDataScrubbers.cs`. Add a test case to `SentryDataScrubbersTests.cs`. That's it.
