...
- [x] `src/PhotoPrint.API/Middleware/ExceptionHandlerMiddleware.cs` — modified: the unhandled-exception branch resolves `IHub` from per-request DI and calls `CaptureException`. *(Superseded by review 044-045-v1 F15: capture is now keyed on the mapped status code, so a mapped **5xx** is captured too — only mappings below 500 are exempt.)*
...
- **Capture only in the `else` branch.** *Superseded by review 044-045-v1 F15.* The intent — business outcomes are not server errors — still holds, but "mapped" turned out to be the wrong proxy for it: `BadGatewayException` maps to 502 and was escaping capture while burning the availability SLO. Capture is now keyed on the mapped status code being ≥ 500, so mappings below 500 (`NotFoundException` etc.) stay exempt while any mapped 5xx is captured and logged at `Error`. *Further qualified by review 044-045-v2 F4:* a caller who disconnects mid-request never becomes a mapped 5xx in the first place — `GoogleTokenValidator` rethrows the cancellation instead of translating it — so the rule reads "any mapped 5xx", not "any 5xx-shaped failure".
