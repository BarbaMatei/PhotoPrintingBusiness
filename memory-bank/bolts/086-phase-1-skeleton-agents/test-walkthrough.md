...
| 3b | The reporting floor applies | **absent** | The same gap story 004 row 3 records: the floor sorts on severity, and no confidence value survives into a published record. The brief fixes the axis on confidence (`bug-hunter-build-guide.md:670`). **Filed in story 010** |
...
1. **Story 006's text is incomplete against its own brief.** Two v3.3 requirements of Prompt 6 —
   ...
   guide (`bug-hunter-build-guide.md:743-746`) and in no acceptance criterion of the story. This
...
4. **The guide's status table** — `general-hunter` ✓ (line 143) and `orchestrator` ✓ (line 144)
   both now have named gaps; the row wording is optimistic rather than wrong. `docs/` is outside
   this bolt's write set.
