...
| **PostgreSQL `LISTEN`/`NOTIFY`** | Sub-second reaction without producer coupling (DB triggers fire on INSERT); decoupled like polling. | Requires Postgres-specific code (no SQLite dev-path equivalent); reconnect logic; not idiomatic in EF Core. | Provider lock-in; sub-second latency is unneeded. |
