...
Confirm on the record, story by story, that the review loop satisfies guide Prompts 1–5 — or name
exactly where it does not. Nothing is built.
...
- [ ] **1. plan** → `implementation-plan.md` (here, the verification plan): read the five stories,
      their `**Status:**` lines, the rows for Prompts 1–5 in the guide's
...
### Requires
- (none — this bolt runs first, before every construction bolt of intent 035)
...
- [ ] All five stories carry a recorded verdict in `test-walkthrough.md`, each with its evidence
      (file:line, command output)
- [ ] No part of the June skeleton was built — no `bug-hunting/` tree, no new ledger or hunter skill
...
- Change anything under `reviews/**`, except this bolt's own review records under
  `reviews/085-phase-1-skeleton-core/` — a needed change to the loop itself is a new story for
  another bolt.
