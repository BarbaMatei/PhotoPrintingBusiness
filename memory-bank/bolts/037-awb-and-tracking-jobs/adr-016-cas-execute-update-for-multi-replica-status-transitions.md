...
| **Postgres advisory locks (`pg_advisory_xact_lock(order_id)`)** | Pessimistic — no race window at all. | Postgres-specific (breaks SQLite dev). Requires manual lock-release reasoning. Sits at the connection layer, awkward to compose with EF. | Stack-coupled and heavier than necessary. |
