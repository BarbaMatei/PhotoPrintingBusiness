...
| A client injects entries through the trusted proxy | Only the rightmost is honoured | `…Trusted_proxy_with_multi_entry_forwarded_for_emits_no_untrusted_warning` | — |
...
| A trusted proxy sends a value no parser can read (`unknown`, an empty value), so the identity silently stays the proxy | Its own Warning, and neither cause's log cap silences the other | `…Trusted_peer_with_unparseable_forwarded_value_is_not_reported_untrusted`, `…Trusted_peer_with_an_empty_forwarded_value_is_not_reported_untrusted`, `…An_exhausted_untrusted_cap_does_not_silence_parse_failures` | `forwarded_headers.unparseable_forwarded_for ip=…` |
| A trusted-proxy entry names a whole subnet, the whole internet, or an IPv6 link-local address | Boot refuses it | `ForwardedHeadersSettingsValidatorTests.A_subnet_wide_range_fails_validation`, `…The_whole_internet_fails_validation`, `…An_ipv6_range_wider_than_one_pair_fails_validation`, `…An_ipv6_link_local_entry_fails_validation` | — |
| Trusted proxies set, observability on, no scrape listener — the one shape where the exclusion cannot fire — and the validator registration is gone | A real boot aborts, naming `Observability:Metrics:ScrapePort` | `ForwardedHeadersWithObservabilityTests.Trusted_proxies_with_observability_on_and_no_scrape_listener_aborts_boot` | `OptionsValidationException` |
| One conjunct of the scrape exclusion is dropped, so ordinary traffic on the scrape port — or `/metrics` on the public port — loses its client identity | Both conjuncts are required; the client still resolves | `…A_non_metrics_path_on_the_scrape_port_still_resolves_its_client`, `…The_metrics_path_on_the_public_port_still_resolves_its_client` | — |
