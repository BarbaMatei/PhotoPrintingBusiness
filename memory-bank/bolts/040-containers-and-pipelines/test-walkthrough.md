...
- **Cross-platform BE fix surfaced by Linux CI:** `UploadServiceTests.UploadAsync_OriginalFileNameContainsPath_StripsDirComponent` passed on the Windows dev box but failed on the Linux runner. Root cause: `UploadService` used `Path.GetFileName(originalFileName)`, which only treats `\` as a separator on Windows — so on the deployed (Linux) server a crafted name like `C:\evil\x.jpg` bypassed directory-stripping (a path-traversal sanitisation gap). Fixed to strip both `/` and `\` OS-independently via `LastIndexOfAny`. Verified: 15/15 `UploadServiceTests` pass. This is exactly the class of latent bug that running the suite on the container's Linux base (bolt 040) exposes.
