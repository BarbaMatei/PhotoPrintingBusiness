...
| Uploads fail with `permission denied` while `/health` stays 200, after upgrading to an image built from a commit at or after this one | The runtime user is now pinned to uid 1001 (`ARG RUNTIME_UID`); an `apidata` volume created by an earlier image is still owned by the base image's own uid, and a build-time `chown` never reaches a volume that already has content | `docker compose -f docker-compose.prod.yml run --rm --user root api chown -R 1001:1001 /app/Storage`, then `up -d` again. |
