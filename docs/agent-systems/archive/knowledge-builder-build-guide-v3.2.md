# Knowledge-Builder System — Complete Build Guide (v3.2, additive)
...
lives in the standalone, normative **`docs/agent-systems/integration-contract-v1.2.md`** — both systems build against
...
> normative contract reference is now **`docs/agent-systems/integration-contract-v1.2.md`**, and the
> bug-hunter spec of record is **`docs/agent-systems/bug-hunter-build-guide-v3.3.md`**.
...
> - **The Integration Contract is extracted** *(F5)* to `docs/agent-systems/integration-contract-v1.2.md` (standalone,
...
v3.2, Integration Contract §2). Superseded,
...
bug-hunting `orchestrator`.* **Method:** (1) **Open:** create the run lock **`knowledge/.run-lock`** (run number, timestamp,
...
the run summary; **write audit (v3.2):** `git status`/diff the tree and fail the run loudly if
anything outside `knowledge/**` (+ the shared code index) changed; **commit the published store to
git** (the publish IS the restore point — v3.2, Integration Contract §5); remove the run lock — on
...
would re-observe them and re-emit forever)**. **Dependencies:**
...
The normative cross-system interface lives in **`docs/agent-systems/integration-contract-v1.2.md`**: storage layout +
...
> record is now `docs/agent-systems/bug-hunter-build-guide-v3.3.md` (review hardening, same day),
...
1. **Part I:** reference `docs/agent-systems/integration-contract-v1.2.md` as normative (shared tools, interleave,
