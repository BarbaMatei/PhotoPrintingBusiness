# Knowledge-Builder System — Complete Build Guide (v3.3, additive)
...
lives in the standalone, normative **`docs/agent-systems/integration-contract-v1.3.md`** — both systems build against
...
> normative contract reference is now **`docs/agent-systems/integration-contract-v1.3.md`**, and the
> bug-hunter spec of record is **`docs/agent-systems/bug-hunter-build-guide-v3.4.md`**.
...
> - **The Integration Contract is extracted** *(F5)* to `docs/agent-systems/integration-contract-v1.3.md` (standalone,
...
the run summary; **write audit (v3.2, scoped v3.3):** diff **only this run's own store paths**
(`git status -- knowledge/` + the shared code index) and fail the run loudly on a write outside the
allowed set — scoping means a sibling system's in-flight files can never trip it (I1); **commit the
published store to git, path-scoped (v3.3):** `git add -- knowledge/` — never `git add -A` — with
the commit window serialized under the cross-system mutex (the publish IS the restore point — v3.2,
Integration Contract §5); remove the run lock — on
...
The normative cross-system interface lives in **`docs/agent-systems/integration-contract-v1.3.md`**: storage layout +
...
> record is now `docs/agent-systems/bug-hunter-build-guide-v3.4.md` (review hardening, same day),
...
1. **Part I:** reference `docs/agent-systems/integration-contract-v1.3.md` as normative (shared tools, interleave,
