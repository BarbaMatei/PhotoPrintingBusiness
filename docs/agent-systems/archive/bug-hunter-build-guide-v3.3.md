# Bug-Hunting Agent System — Complete Build Guide (v3.3, additive)
...
> **`docs/agent-systems/integration-contract-v1.2.md`**.
...
> cross-system interface is now normative in **`docs/agent-systems/integration-contract-v1.2.md`** (storage map, query
...
- **The Integration Contract is normative (v3.1).** Cross-system interfaces — storage layout and the
  ...
  `docs/agent-systems/integration-contract-v1.2.md` and win over any brief in this guide.
...
## Operating the system (v3.3)

- **What starts a run (trigger wiring — pick one and wire it; a cadence policy with no mechanism is how
  the loop starves):** a step in the bolt-completion workflow (alongside
  `.specsmd/aidlc/scripts/bolt-complete.cjs`), a weekly batch, or a pre-merge hook. The fix loop depends
  on runs happening: AI-DLC's "fix done" is only noticed at run open (Prompt 31b).
- **Where runs happen:** only in the designated integration worktree on **`main`** (Integration Contract
  §1 — the stores are single-history; other worktrees treat `bug-hunting/**` as read-only).
- **How to start one:** invoke the `orchestrator` skill with a scope; it takes the run lock, runs the six
  slots, commits the published ledger at close, and releases the lock.
- **Fixed assets you maintain:** the sandbox recipe (Dockerfile / compose + seed data), the decisions
  file `triage-intake` reads, and the eval cadence (run `eval-metrics` after meaningful changes and on
  any model change).
...
the redacted evidence from the record; never reconstruct or emit raw secret material. Then an optional
...
pipeline:** (1) **Open:** create the run lock **`bug-hunting/.run-lock`** (run number, timestamp,
...
**write audit (v3.3):** `git status`/diff the tree and **fail the run loudly** if anything outside the
allowed write set (ledger, reports, fix-requests, code index, sandbox, approved test files) changed;
**commit the published store to git** (the publish IS the restore point — v3.3, Integration Contract
§5); remove the run lock — **on success or abort**. Define
...
(`docs/agent-systems/integration-contract-v1.2.md` §3); keep flow IDs stable across refreshes.
...
regeneration, never by merge). Operations: `find_symbol`, `find_callers`, `find_callees`,
...
directive is quoted and flagged, never obeyed. **Remediation data is third-party input too (v3.3):**
...
(gitleaks, hadolint, checkov, tfsec) via `tool-ingest`, plus reasoning; config/infra file content is
...
envelope: `docs/agent-systems/integration-contract-v1.2.md` §2) for the contracts relevant to a location / flow / symbol;
...
is stale** beyond the threshold (`docs/agent-systems/integration-contract-v1.2.md` §5) before relying on oracle results
(v3.1). **Hunt** — dispatch the specialist hunters (`flow-tracer-agent`, `file-sweeper-agent`,
...
assumed. **Eval isolation (v3.2):** runs over the corpus/fixtures are **eval-mode** — they write to a
...
skill-attributed" (v3.3). **Output:** per-run metrics + trend.
...
re-distilling the fixed bug's contract (`docs/agent-systems/integration-contract-v1.2.md` §4); on a fix that fails its
...
Re-open the `orchestrator` and extend its **Open** step (no restructuring): after taking the run lock
and loading the ledger, **scan `bug-hunting/fix-requests/`** for records with
`fix_status: open | fix-reported`; for each, check the correlated bug-bolt's `bolt.md` in
...
Skip records already terminal (`verified-fixed`, `closed-unverified`); a `fix-failed` record stays
eligible so a re-fix is re-checked. **Tests:** (a) a completed bug-bolt is discovered at run open and
its record moves `open` → `fix-reported` → `verified-fixed`; (b) a record whose bolt is still
in-progress is left at `open`, untouched; (c) a terminal record is not rescanned.
...
`docs/agent-systems/integration-contract-v1.2.md` §4) (v3.1). By AI-DLC convention, the bug-bolt created from this
