# Integration Contract — AI-DLC × Bug-Hunter × Knowledge-Builder (v1.4)
...
> Referenced by: `docs/agent-systems/knowledge-builder-build-guide-v3.4.md` (Appendix A) and
> `docs/agent-systems/bug-hunter-build-guide-v3.5.md` (mirror edits 2026-06-11; G hardening
...
- **Publish = commit (v1.2, owner decision):** every publish of `bug-hunting/**` or `knowledge/**`
  ...
  detection backstop, not the serialization. **The shared code index is never part of any
...
| BH `orchestrator` (incl. Prompt 31b) | §5 staleness warning at run open; §4 fix-request mailbox scan at run open incl. `fix-failed` (v1.3); §1 cross-system mutex + run lock + store-scoped write audit **+ forbidden-ground check (v1.4)**; §5 path-scoped publish-commit (v1.3); §2 `oracle_coverage` warning (v1.3) |
| KB `knowledge-orchestrator` | §1 cross-system mutex + run lock + store-scoped write audit **+ forbidden-ground check (v1.4)**; §5 path-scoped publish-commit + rollback (v1.3) |
