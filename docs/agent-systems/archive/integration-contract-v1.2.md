# Integration Contract — AI-DLC × Bug-Hunter × Knowledge-Builder (v1.2)
...
> Referenced by: `docs/agent-systems/knowledge-builder-build-guide-v3.2.md` (Appendix A) and
> `docs/agent-systems/bug-hunter-build-guide-v3.3.md` (mirror edits 2026-06-11; G hardening
> 2026-06-11; H mechanisms 2026-06-12).
...
| `bug-hunting/code-index/` (symbol/reference index) | the shared `code-index` tool — derived, regenerable; either system's run may refresh it (the one sanctioned dual-writer; conflicts resolved by regeneration, never merge) (v1.2) | both systems |
...
its allowed set (v1.2). Each ledger records a content
...
tamper_warning? }` — `tamper_warning` (v1.2) is set when the ledger loaded with a content-hash
mismatch (§1); consumers treat such results as degraded (corroborating like
...
- **"Verified fixed" (bug-hunter):** the fix-request record in `bug-hunting/fix-requests/` carries
  ...
  no-proving-test fallback closure, excluded from the run-open scan (so statically-confirmed fixes
  don't park at `fix-reported` forever); the knowledge builder does **not** re-distil on it (no
  proof). All transitions after `open` are written by the bug-hunter's
...
- **Checking mechanisms (v1.1)** — a mailbox nobody polls starves silently, so both directions are
  ...
  orchestrator extension — v1.2; previously this mechanism had no builder). **AI-DLC discovers new fix-requests** through the
...
- **Publish = commit (v1.2, owner decision):** every publish of `bug-hunting/**` or `knowledge/**`
  ...
  — the commit IS the restore point. On a corrupt or hash-mismatched load, the ledger-io `load`
...
| BH `orchestrator` (incl. Prompt 31b) | §5 staleness warning at run open; §4 fix-request mailbox scan at run open (built by 31b — v1.2); §1 run lock, write audit, single-history rule; §5 publish-commit (v1.2) |
| KB `knowledge-orchestrator` | §1 run lock, write audit, single-history rule; §5 publish-commit + rollback (v1.2) |
