# Bug-Hunting Agent System — Complete Build Guide (v3.4, additive)
...
> **`docs/agent-systems/integration-contract-v1.3.md`**.
...
> cross-system interface is now normative in **`docs/agent-systems/integration-contract-v1.3.md`** (storage map, query
...
- **The Integration Contract is normative (v3.1).** Cross-system interfaces — storage layout and the
  ...
  `docs/agent-systems/integration-contract-v1.3.md` and win over any brief in this guide.
...
- **What starts a run (trigger wiring — pick one and wire it; a cadence policy with no mechanism is how
  ...
  `.specsmd/aidlc/scripts/bolt-complete.cjs`), a weekly batch, or a pre-merge hook — where a
  **pre-merge run is read-only advisory (v3.4)**: it writes no ledger, coverage, or mailbox state
  (those writes belong to the integration worktree only, per the single-history rule); its findings
  go to the PR comment alone. The fix loop depends
...
**write audit (v3.3, scoped v3.4):** diff **only this run's own store paths**
(`git status -- bug-hunting/` + the code index) against the allowed write set (ledger, reports,
fix-requests, code index, sandbox, approved test files) and **fail the run loudly** on a violation —
scoping means a sibling system's in-flight files can never trip this audit (I1);
**commit the published store to git, path-scoped (v3.4):** `git add -- bug-hunting/` — never
`git add -A` — with the commit window serialized under the cross-system mutex, so the restore point
is always one system's clean publish (the publish IS the restore point — v3.3, Integration Contract
...
(`docs/agent-systems/integration-contract-v1.3.md` §3); keep flow IDs stable across refreshes.
...
staleness is detectable, mirroring `app-mapping`'s freshness stamp. Operations: `find_symbol`, `find_callers`, `find_callees`,
...
envelope: `docs/agent-systems/integration-contract-v1.3.md` §2) for the contracts relevant to a location / flow / symbol;
...
is stale** beyond the threshold (`docs/agent-systems/integration-contract-v1.3.md` §5) before relying on oracle results
...
re-distilling the fixed bug's contract (`docs/agent-systems/integration-contract-v1.3.md` §4); on a fix that fails its
...
`docs/agent-systems/integration-contract-v1.3.md` §4) (v3.1). By AI-DLC convention, the bug-bolt created from this
