...
- Tests named in a bolt's `failure-modes.jsonl` run red first, then green, then red again under `--kind revert-and-rerun --mutate <file>:<line>`, all with `--log memory-bank/bolts/<bolt-id>/test-stamps.jsonl` instead of `--no-events`; `node .specsmd/aidlc/scripts/check-stage-exit.mjs <bolt-id> <stage>` says what is still missing (`bolt-process.md`, "Stage exit conditions").
