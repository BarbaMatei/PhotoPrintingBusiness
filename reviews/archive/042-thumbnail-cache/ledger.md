...
|---|---|---|---|---|---|---|
| PPW-52 | 🔴 | v1 | Preview `Cache-Control: public` on an ownership-checked response leaks it between users | `Controllers/UploadsController.cs:126` | verified | `095285c` |
| PPW-53 | 🔴 | v1 | The decode-bomb guard checks each axis, so it misses total pixels and frame count | `Services/ImageProcessor.cs:47` | verified | `095285c` |
| PPW-54 | 🔴 | v1 | The guest-401 self-heal branch of the interceptor has no test | `UI/…/error.interceptor.ts:27` | verified | `095285c` |
| PPW-55 | 🟠 | v1 | The cleanup job never deletes `ThumbnailPath`, so thumbnails pile up forever | `BackgroundJobs/UploadCleanupJob.cs:90` | verified | `095285c` |
| PPW-56 | 🟠 | v1 | The cache-fill write is neither repeatable nor atomic, so it orphans thumbnails | `Services/UploadService.cs:145` | verified | `095285c` |
| PPW-57 | 🟡 | v1 | An unreadable image at preview time is unmapped and returns 500 instead of 422 | `Services/ImageProcessor.cs:46` | verified | `095285c` |
| PPW-58 | 🟠 | v1 | Story 003's memory-allocator cap was dropped with no equivalent | `Program.cs` | verified | `095285c` |
| PPW-59 | 🟡 | v1 | The thumbnail is saved at a random path, not the spec's id-keyed path | `Services/UploadService.cs:145` | verified | `095285c` |
| PPW-60 | 🟡 | v1 | Story 002's soft-delete case contradicts the implemented and tested 404 | `Services/UploadService.cs:128` | verified | `095285c` |
| PPW-61 | 🟡 | v1 | The bundled guest-auth and dev-warning changes have no story, criterion or test | `Program.cs` | verified | `095285c` |
| PPW-62 | 🟠 | v1 | Two overlapping `ensureGuestSession` calls mint two guest sessions | `UI/…/format-selector-page.ts:184` | verified | `095285c` |
| PPW-63 | 🟠 | v1 | The self-heal is not seamless: a stale token fails the first upload with no retry | `UI/…/format-selector-page.ts:168` | verified | `095285c` |
| PPW-64 | 🟡 | v1 | A visitor with no guest token is logged out to a login page they cannot use | `UI/…/error.interceptor.ts:27` | verified | `095285c` |
| PPW-65 | 🟡 | v1 | `restoreFromSession` wipes the restored grid on an expired-token 401 | `UI/…/format-selector-page.ts:347` | verified | `095285c` |
| PPW-66 | 🟠 | v1 | Batch-upload rejections are swallowed with no logging | `Controllers/UploadsController.cs:98` | verified | `095285c` |
| PPW-67 | 🟡 | v1 | The client-cancellation log sits at Debug, under the Information floor, so it never emits | `Middleware/ExceptionHandlerMiddleware.cs:54` | verified | `095285c` |
| PPW-68 | 🟡 | v1 | A pixel-bomb 422 reads in the logs exactly like an ordinary unreadable image | `Services/ImageProcessor.cs:48` | verified | `095285c` |
| PPW-69 | 🟡 | v1 | `AsNoTracking` was dropped, so every cache hit change-tracks for nothing | `Services/UploadService.cs:127` | verified | `095285c` |
| PPW-70 | 🟡 | v1 | The miss branch throws away the generated thumbnail and re-reads it from storage | `Services/UploadService.cs:143` | verified | `095285c` |
...
| PPW-78 | 🟡 | v1 | Cache-Control, 304, the migration and the cache-miss race have no tests | `Tests/…` | verified | `095285c` |
| PPW-79 | 🟡 | v1 | The storage contract assumes a rewindable stream with a readable length | `Controllers/UploadsController.cs:155` | backlog | `bd0d5fd` |
| PPW-80 | 🟠 | v2 | The 50 MP decode cap refuses legitimate large-format print uploads | `Services/ImageProcessor.cs` | verified | `f8b1325` |
| PPW-81 | 🟡 | v2 | A restored upload is discarded on any non-401 preview error, including a passing blip | `UI/…/format-selector-page.ts` | verified | `f8b1325` |
| PPW-82 | 🟡 | v2 | Nothing reclaims a thumbnail written between the cleanup job's read and its commit | `BackgroundJobs/UploadCleanupJob.cs:101` | backlog | `bd0d5fd` |
| PPW-83 | 🟡 | v2 | Stored keys use the operating system's separator instead of a forward slash | `Services/LocalStorageService.cs` | verified | `f8b1325` |
| PPW-84 | 🟠 | v4 | Image decode has no total or concurrent memory bound, so many large images exhaust memory | `Services/UploadService.cs:158` | verified | `6c4f334` |
| PPW-85 | 🟠 | v4 | The cache-fill write races the cleanup job and strands a thumbnail on the dead row | `Services/UploadService.cs:216` | backlog | `bd0d5fd` |
| PPW-86 | 🟠 | v4 | Two first previews at once collide on an exclusive file create and return 500 | `Services/LocalStorageService.cs:32` | verified | `6c4f334` |
| PPW-87 | 🟠 | v4 | A bomb sent to the batch endpoint never emits the reserved alert event | `Controllers/UploadsController.cs:119` | verified | `6c4f334` |
| PPW-88 | 🟠 | v4 | HEIC is accepted but no decoder exists, so every HEIC upload fails | `Services/MimeValidator.cs:52` | verified | `6c4f334` |
| PPW-89 | 🟠 | v4 | A cache miss whose original is gone returns 500 instead of a clean 4xx | `Services/ImageProcessor.cs:63` | verified | `6c4f334` |
| PPW-90 | 🟠 | v4 | An unreadable stored image is logged without its storage path or its cause | `Services/ImageProcessor.cs:88` | verified | `6c4f334` |
| PPW-91 | 🟠 | v4 | A preview kept on 403 leaves an upload the guest can never put in a cart | `UI/…/format-selector-page.ts:400` | verified | `6c4f334` |
| PPW-92 | 🟠 | v4 | No test proves the bomb rejection deletes the file it already stored | `Tests/…/UploadServiceTests.cs:381` | verified | `6c4f334` |
| PPW-93 | 🟠 | v4 | The one-frame decode cap is proven only through the internal helper, not the public call | `Services/ImageProcessor.cs:81` | backlog | `bd0d5fd` |
| PPW-94 | 🟡 | v4 | The cache-hit path checks then reads, so a vanished file gives 500 and costs a round-trip | `Services/UploadService.cs:150` | verified | `6c4f334` |
| PPW-95 | 🟡 | v4 | A vanished cache file quietly regenerates with no signal | `Services/UploadService.cs:150` | verified | `6c4f334` |
| PPW-96 | 🟡 | v4 | A thumbnail orphaned by a failed commit emits no signal | `Services/UploadService.cs:159` | verified | `6c4f334` |
| PPW-97 | 🟡 | v4 | The preview GET writes to the database, which fails against a read replica | `Services/UploadService.cs:166` | backlog | `6c4f334` |
| PPW-98 | 🟡 | v4 | The batch-rejection log prints the raw client filename with no length limit | `Controllers/UploadsController.cs:120` | verified | `6c4f334` |
| PPW-99 | 🟡 | v4 | Any unauthenticated 401 wipes the whole guest session, checkout contact details included | `UI/…/error.interceptor.ts:30` | verified | `79c2eda` |
| PPW-100 | 🟡 | v4 | The one-shot retry guard is untested for a retry that still fails | `UI/…/format-selector-page.ts:216` | verified | `6c4f334` |
| PPW-101 | 🟡 | v4 | Guest-session recovery after a failed init is untested | `UI/…/format-selector-page.ts:214` | backlog | `bd0d5fd` |
| PPW-102 | 🟡 | v4 | The bomb log test asserts the event name but not the dimensions the event exists to carry | `Tests/…/ExceptionHandlerMiddlewareTests.cs:254` | verified | `6c4f334` |
| PPW-103 | 🟡 | v4 | The 512 MB allocator backstop throws an unmapped exception, giving 500, and is untested | `Program.cs:95` | verified | `6c4f334` |
| PPW-104 | 🟡 | v4 | The recognised-but-broken image path to 422 is untested | `Services/ImageProcessor.cs:81` | verified | `6c4f334` |
| PPW-105 | ⚪ | v4 | Preview object URLs are never released, so every restore and retry leaks one | `UI/…/format-selector-page.ts:388` | verified | `6c4f334` |
| PPW-106 | ⚪ | v4 | The upload error message is written out at three sites | `UI/…/format-selector-page.ts:220` | verified | `6c4f334` |
| PPW-107 | ⚪ | v4 | The self-heal seam is tested only with each half mocked | `UI/…/format-selector-page.ts:227` | verified | `6c4f334` |
| PPW-108 | ⚪ | v4 | The implementation walkthrough contradicts the shipped code | `memory-bank/…/implementation-walkthrough.md:32` | fixed | `838c9b6` |
| PPW-109 | ⚪ | v4 | Story 003 says 54 MP is refused while the shipped cap is 100 MP | `memory-bank/…/003-imagesharp-max-pixels.md:27` | verified | `6c4f334` |
| PPW-110 | ⚪ | v4 | Story 001 names `varchar(500)` and `StoragePath`; the column shipped as `varchar(512)` and `FilePath` | `memory-bank/…/001-thumbnail-path-schema.md:22` | verified | `bd0d5fd` |
| PPW-111 | ⚪ | v4 | The thumbnail shipped at 300 px while the stories and the brief say 800 px | `memory-bank/…/002-persist-thumbnail-on-first-request.md:39` | verified | `6c4f334` |
| PPW-112 | 🟠 | v6 | The decode limiter defaults to the core count and ignores memory, so it still exhausts memory | `Program.cs:359` | verified | `79c2eda` |
| PPW-113 | 🟠 | v6 | A bomb caught by the allocator backstop never emits the reserved bomb event | `Middleware/ExceptionHandlerMiddleware.cs:106` | verified | `79c2eda` |
| PPW-114 | 🟠 | v6 | A logged-in user whose token expired is re-attributed to a throwaway guest | `UI/…/format-selector-page.ts:232` | verified | `79c2eda` |
| PPW-115 | 🟠 | v6 | The HEIC removal is missing from the bolt's bundled-scope document | `memory-bank/…/bolt.md:57` | verified | `79c2eda` |
| PPW-116 | 🟠 | v6 | The test walkthrough certifies a `Cache-Control` value the code never emits | `memory-bank/…/test-walkthrough.md:28` | verified | `79c2eda` |
| PPW-117 | 🟡 | v6 | `ExistsAsync` was added to the storage interface but nothing in production calls it | `IStorageService.cs:21` | backlog | `bd0d5fd` |
| PPW-118 | 🟡 | v6 | Every cache-miss preview pays an extra database round-trip to spot the soft-delete race | `Services/UploadService.cs:216` | backlog | `bd0d5fd` |
| PPW-119 | 🟡 | v6 | Nothing reports how saturated or how queued the decode limiter is | `ImageDecodeLimiter.cs:30` | backlog | `bd0d5fd` |
| PPW-120 | 🟡 | v6 | No test proves the decode slot is released when the decode throws | `Services/ImageProcessor.cs:67` | backlog | `bd0d5fd` |
| PPW-121 | 🟡 | v6 | The allocator-exception-to-422 mapping is proven only by an injected instance | `Middleware/ExceptionHandlerMiddleware.cs:26` | backlog | `79c2eda` |
| PPW-122 | 🟡 | v6 | A failed thumbnail delete in the cleanup job is untested and silently leaks the file again | `BackgroundJobs/UploadCleanupJob.cs:114` | backlog | `79c2eda` |
| PPW-123 | 🟡 | v6 | Parallel preview 401s defeat the init sharing, and a late 401 wipes a fresh token | `UI/…/format-selector-page.ts:381` | backlog | `79c2eda` |
| PPW-124 | 🟡 | v6 | The logged-in 401-during-upload path has no test | `UI/…/error.interceptor.ts:24` | verified | `79c2eda` |
| PPW-125 | 🟡 | v6 | The guest-init error path in `onFilesAccepted` is untested, so files hang as uploading | `UI/…/format-selector-page.ts:176` | backlog | `79c2eda` |
| PPW-126 | 🟡 | v6 | Moving a file onto a shared key races other writers on Windows and returns 500 | `Services/LocalStorageService.cs:45` | backlog | `bd0d5fd` |
| PPW-127 | 🟡 | v6 | A cleanup delete fails against an open read handle on Windows and leaves an orphan | `Services/LocalStorageService.cs` | backlog | `79c2eda` |
| PPW-128 | 🟠 | v6 | The pixel-area cap ignores bytes per pixel, so a legitimate 16-bit PNG is refused | `Services/ImageProcessor.cs:23` | verified | `bd0d5fd` |
| PPW-129 | 🟡 | v6 | The pixel guard is skipped when the identify call returns null | `Services/ImageProcessor.cs:77` | false-positive | `e2093bd` |
| PPW-130 | 🟡 | v6 | Storage faults and cancellation are reported as an unreadable image | `Services/ImageProcessor.cs:56` | backlog | `79c2eda` |
| PPW-131 | 🟡 | v6 | The implementation plan's acceptance criteria are stale after documented substitutions | `memory-bank/…/implementation-plan.md:59` | backlog | `79c2eda` |
| PPW-132 | ⚪ | v6 | The bomb-alert log template is copied across the controller and the middleware | `Controllers/UploadsController.cs:122` | backlog | `bd0d5fd` |
| PPW-133 | ⚪ | v6 | `dropRestoredEntry` repeats the body of `onRemoveUpload` word for word | `UI/…/format-selector-page.ts:420` | backlog | `79c2eda` |
| PPW-134 | ⚪ | v6 | The client-abort log reads the raw correlation-id item instead of the accessor | `Middleware/ExceptionHandlerMiddleware.cs:64` | backlog | `79c2eda` |
| PPW-135 | ⚪ | v6 | Storage save and delete traces sit at Debug under an Information floor, so they never emit | `Services/LocalStorageService.cs:53` | backlog | `79c2eda` |
| PPW-136 | 🟠 | v8 | The global split-query default mis-pages a collection include that has no tiebreaker | `Services/AdminOrderService.cs:67` | verified | `bd0d5fd` |
| PPW-137 | 🟠 | v8 | `storeSession` overwrites the contact details the earlier fix preserved | `UI/…/format-selector-page.ts:205` | verified | `bd0d5fd` |
| PPW-138 | 🟠 | v8 | The bomb test asserts the base exception, so the alert can regress while tests stay green | `Tests/…/UploadServiceTests.cs:480` | verified | `bd0d5fd` |
| PPW-139 | 🟠 | v8 | A lost original blob is logged as a plain 404, with no distinct signal | `Services/UploadService.cs:183` | verified | `bd0d5fd` |
| PPW-140 | 🟠 | v8 | The soft-delete-race deletion leaves database and file state silently out of step | `Services/UploadService.cs:219` | verified | `bd0d5fd` |
| PPW-141 | 🟡 | v8 | The 30-day private preview cache is recoverable on a shared device | `Controllers/UploadsController.cs:26` | backlog | `bd0d5fd` |
| PPW-142 | 🟡 | v8 | Bundled change C has no criterion or test and is labelled as changing no behaviour | `memory-bank/…/bolt.md:73` | verified | `bd0d5fd` |
| PPW-143 | 🟡 | v8 | A restore preview that resolves after the page is destroyed leaks an object URL | `UI/…/format-selector-page.ts:404` | backlog | `bd0d5fd` |
| PPW-144 | 🟡 | v8 | No end-to-end test reaches the bomb-to-422 path because the integration fake pins 800×600 | `Tests/…/UploadFactory.cs:239` | backlog | `bd0d5fd` |
| PPW-145 | 🟡 | v8 | A guest 401 away from the upload page is a silent dead end | `UI/…/error.interceptor.ts:33` | backlog | `bd0d5fd` |
| PPW-146 | 🟡 | v8 | `localUrl()` mints an untracked object URL on every change-detection cycle | `UI/…/photo-thumbnail.component.ts:86` | backlog | `bd0d5fd` |
| PPW-147 | 🟡 | v8 | The decode memory budget ignores the upload buffering that shares the same memory | `ImageDecodeLimiter.cs:30` | backlog | `bd0d5fd` |
| PPW-148 | ⚪ | v8 | The conditional GET matches only an exact strong tag, so weak, list and `*` fall back to 200 | `Controllers/UploadsController.cs:158` | backlog | `bd0d5fd` |

## Details

### PPW-52 — Preview `Cache-Control: public` on an ownership-checked response leaks it between users
...
- **History:**
  - v1: found — one of the pass's three fixes required before merge
...
### PPW-53 — The decode-bomb guard checks each axis, so it misses total pixels and frame count
...
- **History:**
  - v1: found — five lenses reached it independently and each built the exploit
...
### PPW-54 — The guest-401 self-heal branch of the interceptor has no test
...
### PPW-55 — The cleanup job never deletes `ThumbnailPath`, so thumbnails pile up forever
...
### PPW-56 — The cache-fill write is neither repeatable nor atomic, so it orphans thumbnails
...
- **History:**
  ...
  - v2: verified @`095285c`
  - v4: recorded as the cause of PPW-85, PPW-86 and PPW-89, which the deterministic key created

### PPW-57 — An unreadable image at preview time is unmapped and returns 500 instead of 422
...
- **History:**
  - v1: found — the residual of a suspicion the pass refuted
  - round 1: fixed @`533996c` — the generic image-format exception is caught and mapped to 422
  - v2: verified @`095285c`

### PPW-58 — Story 003's memory-allocator cap was dropped with no equivalent
...
- **What:** The story required both an allocator cap and per-image dimension limits. The dimension limits
  ...
  PPW-53's bomb allocate gigabytes.
- **History:**
  - v1: found
  - round 1: fixed @`533996c` — a 512 MB allocator cap, with the story amended @`eb8a6f8`
  - v2: verified @`095285c`

### PPW-59 — The thumbnail is saved at a random path, not the spec's id-keyed path

- **What:** Story 002 specifies a path derived from the upload id. The code minted a random name, which
  fed PPW-55 and PPW-56 and would complicate the later move to cloud storage.
- **History:**
  - v1: found
  - round 1: fixed @`c245a1e` — owner-scoped `thumbs/{ownerId}/{uploadId}.jpg`, story amended @`eb8a6f8`
  - v2: verified @`095285c`

### PPW-60 — Story 002's soft-delete case contradicts the implemented and tested 404
...
### PPW-61 — The bundled guest-auth and dev-warning changes have no story, criterion or test
...
### PPW-62 — Two overlapping `ensureGuestSession` calls mint two guest sessions
...
### PPW-63 — The self-heal is not seamless: a stale token fails the first upload with no retry
...
### PPW-64 — A visitor with no guest token is logged out to a login page they cannot use
...
### PPW-65 — `restoreFromSession` wipes the restored grid on an expired-token 401
...
### PPW-66 — Batch-upload rejections are swallowed with no logging
...
### PPW-67 — The client-cancellation log sits at Debug, under the Information floor, so it never emits
...
### PPW-68 — A pixel-bomb 422 reads in the logs exactly like an ordinary unreadable image
...
### PPW-69 — `AsNoTracking` was dropped, so every cache hit change-tracks for nothing
...
### PPW-70 — The miss branch throws away the generated thumbnail and re-reads it from storage
...
### PPW-71 — The dimension check and its message are duplicated across two layers
...
### PPW-72 — The 30-day cache lifetime is the inline number `2592000`
...
### PPW-73 — Split-query configuration is written out separately in both database branches
...
### PPW-74 — The migration's Postgres arm and the model snapshot are exercised by no test
...
- **History:**
  - v1: found — the provider-aware column type was fixed @`bca68fa`; the test was deferred
...
- **History:**
  ...
  - v2: verified @`095285c` — a legitimate HEIC still fails to decode, tracked separately as PPW-88

### PPW-76 — The real image processor, and so the bomb guard, is mocked in every test

- **What:** No test exercised the real processor. The identify guard, the stream rewind and the decode
  itself ran in no test, so PPW-53 could return silently.
- **History:**
  - v1: found
  - round 1: fixed @`533996c` — tests against the real processor for bomb, valid and unreadable inputs
  - v2: verified @`095285c`

### PPW-77 — Cache persistence is unproven because one database context is shared across both calls
...
### PPW-78 — Cache-Control, 304, the migration and the cache-miss race have no tests
...
- **History:**
  - v1: found
  - round 1: fixed @`fad7693` — tests added alongside each matching fix; the migration test stayed with PPW-74
  - v2: verified @`095285c`

### PPW-79 — The storage contract assumes a rewindable stream with a readable length
...
- **History:**
  - v1: found — deferred; no cloud provider existed to trigger it
...
- **What:** PPW-53's cap was correct defence but a behaviour change for a printing product: an A1 poster at
  300 dots per inch, and ordinary high-resolution phone photos, were refused at upload.
- **History:**
  - v2: found — raised by the verification pass as a consequence of the PPW-53 fix
  - round 2: fixed @`656c2fd` — owner raised the cap to 100 MP, which stays under the allocator cap
  - v3: verified @`f8b1325`

### PPW-81 — A restored upload is discarded on any non-401 preview error, including a passing blip
...
### PPW-82 — Nothing reclaims a thumbnail written between the cleanup job's read and its commit
...
- **History:**
  - v2: found — deferred to the cloud-storage bolt, where the storage lifecycle is redesigned
  - v5: the M1 residual was ruled a wont-fix backed by this row
...
### PPW-84 — Image decode has no total or concurrent memory bound, so many large images exhaust memory
...
- **History:**
  ...
  - v5: verified @`6c4f334`
  - v6: the limiter's default was found still unsafe and recorded separately as PPW-112

### PPW-85 — The cache-fill write races the cleanup job and strands a thumbnail on the dead row
...
- **History:**
  - v4: found — the residual of the PPW-56 deterministic-key fix
...
- **What:** The deterministic key from the PPW-56 fix meant two simultaneous first previews wrote the same
...
- **History:**
  ...
  - v5: verified @`6c4f334`
  - v6: the move itself was found to race on Windows and recorded separately as PPW-126

### PPW-87 — A bomb sent to the batch endpoint never emits the reserved alert event
...
### PPW-88 — HEIC is accepted but no decoder exists, so every HEIC upload fails
...
- **History:**
  - v4: found — the residual left open when PPW-75 fixed the container check
  - round 4: fixed @`80379f6` and @`63b815a` — the format is no longer advertised, backend and interface
  - v5: verified @`6c4f334`
  - v6: the missing scope document for this removal was recorded separately as PPW-115

### PPW-89 — A cache miss whose original is gone returns 500 instead of a clean 4xx
...
### PPW-90 — An unreadable stored image is logged without its storage path or its cause
...
### PPW-91 — A preview kept on 403 leaves an upload the guest can never put in a cart
...
### PPW-92 — No test proves the bomb rejection deletes the file it already stored
...
### PPW-93 — The one-frame decode cap is proven only through the internal helper, not the public call
...
- **History:**
  - v4: found — the cap had no coverage at all
...
### PPW-95 — A vanished cache file quietly regenerates with no signal
...
### PPW-96 — A thumbnail orphaned by a failed commit emits no signal
...
### PPW-97 — The preview GET writes to the database, which fails against a read replica
...
- **What:** The warning added by PPW-66 wrote the client's filename straight into the log, with no bound on
  length and no stripping of control characters.
- **History:**
  - v4: found — the residual of the PPW-66 fix
  - round 4: fixed @`158b733` — control characters stripped and the name capped at 128 characters
  - v5: verified @`6c4f334`

### PPW-99 — Any unauthenticated 401 wipes the whole guest session, checkout contact details included

- **What:** The PPW-64 fix cleared the guest session on every unauthenticated 401, anywhere in the
...
- **History:**
  - v4: found — disputed, because it read as a request to revert the verified PPW-64 decision
  - v6: re-raised, sharpened to the contact-details loss rather than the login redirect
  ...
  - v8: a second writer was found to undo the same preservation and is recorded separately as PPW-137

### PPW-100 — The one-shot retry guard is untested for a retry that still fails
...
### PPW-101 — Guest-session recovery after a failed init is untested

- **What:** The shared in-flight init from PPW-62 resets when it settles, so a caller after a failure should
...
- **History:**
  - v4: found — the re-init-after-settle path had no test
...
### PPW-103 — The 512 MB allocator backstop throws an unmapped exception, giving 500, and is untested
...
- **History:**
  ...
  - v5: verified @`6c4f334`
  - v6: the missing bomb event on this branch was recorded separately as PPW-113

### PPW-104 — The recognised-but-broken image path to 422 is untested
...
### PPW-105 — Preview object URLs are never released, so every restore and retry leaks one
...
- **History:**
  ...
  - v5: verified @`6c4f334`
  - v8: two remaining leak paths were recorded separately as PPW-143 and PPW-146

### PPW-106 — The upload error message is written out at three sites
...
### PPW-107 — The self-heal seam is tested only with each half mocked
...
### PPW-108 — The implementation walkthrough contradicts the shipped code

- **What:** The walkthrough described the insecure cache directive PPW-52 replaced, the tracking behaviour
  PPW-69 restored, and the pre-fix migration. Copying from it would have reintroduced PPW-52.
...
- **History:**
  ...
  - v5: verified in part; three residual contradictions were named and were outside the fix
...
### PPW-109 — Story 003 says 54 MP is refused while the shipped cap is 100 MP

- **What:** The acceptance criterion still quoted the figure from before PPW-80 raised the cap.
- **History:**
  - v4: found
  - round 4: fixed @`6c4f334` — the criterion quotes 110 MP over the 100 MP cap
  - v5: verified @`6c4f334`

### PPW-110 — Story 001 names `varchar(500)` and `StoragePath`; the column shipped as `varchar(512)` and `FilePath`
...
- **History:**
  ...
  - v5: verified @`6c4f334`
  - v8: re-raised — the same stale width survived in two more documents
...
### PPW-111 — The thumbnail shipped at 300 px while the stories and the brief say 800 px
...
- **History:**
  ...
  - v5: verified @`6c4f334`, with one stale interface comment left behind
  - round 5: the interface comment corrected @`838c9b6`

### PPW-112 — The decode limiter defaults to the core count and ignores memory, so it still exhausts memory

- **What:** The PPW-84 limiter defaulted its slot count to the processor count. On a host with many cores
  and little memory the default still allowed enough concurrent decodes to exhaust it.
- **History:**
  - v6: found — the residual of the PPW-84 fix
  ...
  - v8: the budget's blind spot for upload buffering was recorded separately as PPW-147

### PPW-113 — A bomb caught by the allocator backstop never emits the reserved bomb event

- **What:** PPW-103 mapped the allocator's memory exception to 422 but did not emit the reserved bomb event,
  so a bomb caught by the backstop rather than the pixel guard was invisible to alerting.
- **History:**
  - v6: found — the residual of the PPW-103 fix
  - round 6: fixed @`6b7ce09` — the middleware emits the event, tagged with which guard caught it
  - v7: verified @`79c2eda`
  - v8: the third copy of the event template was recorded against PPW-132

### PPW-114 — A logged-in user whose token expired is re-attributed to a throwaway guest
...
### PPW-115 — The HEIC removal is missing from the bolt's bundled-scope document

- **What:** PPW-88 stopped accepting a previously advertised upload format. That is a contract change and it
...
### PPW-116 — The test walkthrough certifies a `Cache-Control` value the code never emits

- **What:** A second document, separate from PPW-108, still stated the public cache directive that PPW-52
...
### PPW-117 — `ExistsAsync` was added to the storage interface but nothing in production calls it

- **What:** The PPW-94 fix removed the only production caller of the existence check, leaving an interface
...
- **Suggested fix:** Drop the member, or record why the cloud provider will need it, and delete the
  stubs either way.
- **History:**
  - v6: found — deferred to the cloud-storage bolt, where the interface changes
...
- **What:** The PPW-85 fix re-reads the row after the write to see whether it is still live. That is an extra
  query on every miss, and it disappears only when PPW-85's conditional update lands.
- **Evidence:** `Services/UploadService.cs:216`; the row is already loaded before the write (v8 F11).
- **Suggested fix:** Fold it into PPW-85's conditional update, which removes the second read.
- **History:**
  - v6: found — the residual of the PPW-85 fix; deferred, paired with PPW-85
...
- **What:** The limiter added by PPW-84 is a hard cap on throughput. It emits nothing, so operators cannot
...
- **What:** The map in PPW-103 keys on the exact exception type. The test injects an instance of that type,
...
- **What:** The PPW-55 delete is inside a catch that counts file errors and soft-deletes the row anyway. When
  the delete fails, the file leaks exactly as before PPW-55, and no test covers that branch.
...
- **Suggested fix:** A test with a throwing storage service, asserting the failure is counted, plus the
  PPW-82 sweep as the durable backstop.
- **History:**
...
- **What:** PPW-114's path — a signed-in user whose token expires mid-upload — had no coverage, so the
  interceptor's logout could race the page's retry unnoticed.
- **History:**
  - v6: found
  - round 6: fixed @`39b0098` — the PPW-114 regression tests assert no guest session is minted and no retry runs
  - v7: verified @`79c2eda` — the reclassification was checked and matches what the finding asked for

### PPW-125 — The guest-init error path in `onFilesAccepted` is untested, so files hang as uploading
...
- **What:** The PPW-86 fix moves a temporary file onto the shared key. On Windows that move can fail while
...
- **Suggested fix:** The PPW-82 sweep covers it; otherwise open reads with sharing that permits deletion.
- **History:**
...
- **History:**
  - v6: found as a 🟡; deferred to the next pass
  - v8: re-raised and raised to 🟠 once the permanent failure was traced
...
### PPW-129 — The pixel guard is skipped when the identify call returns null
...
- **History:**
  - v6: found — recorded as plausible but dead today
...
### PPW-130 — Storage faults and cancellation are reported as an unreadable image
...
- **What:** The plan still lists the public cache directive and the per-axis dimension cap, both replaced
  during the first fix round. Same drift family as PPW-108 and PPW-116, in a third document.
...
- **What:** The reserved bomb event name and its fields are written out at three sites after the PPW-113 fix,
...
- **What:** The PPW-91 fix added a second function whose body duplicates an existing one.
...
- **What:** The same class of defect as PPW-67, one file over: storage traces are written at a level the
...
- **History:**
  - v8: found — the pass's headline new finding, on a surface no earlier pass audited
  ...
  - v8: recorded limitation — the in-memory test provider does not split queries, so the missing-items
    symptom itself can only be reproduced on Postgres. That verification rides with PPW-74

### PPW-137 — `storeSession` overwrites the contact details the earlier fix preserved

- **What:** The PPW-99 fix stopped the interceptor wiping contact details, but the re-init path writes the
...
- **History:**
  - v8: found — the residual that showed fixes were still making new defects
...
### PPW-138 — The bomb test asserts the base exception, so the alert can regress while tests stay green
...
### PPW-139 — A lost original blob is logged as a plain 404, with no distinct signal

- **What:** The PPW-89 fix turned a missing original into a 404. A missing blob under a live row is a
...
### PPW-140 — The soft-delete-race deletion leaves database and file state silently out of step

- **What:** When PPW-85's liveness re-read finds the row dead it deletes the thumbnail, but the dead row
  keeps the path it just wrote and nothing says so.
- **History:**
  - v8: found
  - round 8: fixed @`521fa15` — a distinct warning on that branch
  - v9: verified @`bd0d5fd`. The stale path on the dead row is PPW-85's deferred work, not this signal

### PPW-141 — The 30-day private preview cache is recoverable on a shared device

- **What:** The PPW-52 fix made the preview cache private, which stops shared caches but not the browser's
...
- **What:** The bundled split-query default was documented as changing no behaviour. PPW-136 shows it changes
...
- **History:**
  ...
  - round 8: fixed @`76d0b6a` — the document states it is a query-execution change and carries a
    retroactive criterion requiring a unique ordering tiebreaker, naming the PPW-136 test
  - v9: verified @`bd0d5fd` — the cited test exists and the Postgres limitation is stated correctly

### PPW-143 — A restore preview that resolves after the page is destroyed leaks an object URL
...
- **What:** The PPW-112 budget divides host memory by a per-decode allowance, but uploads buffered in memory
