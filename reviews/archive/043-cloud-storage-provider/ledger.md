...
|---|---|---|---|---|---|---|
| PPW-149 | 🔴 | v1 | Admin ZIP fulfilment download reads promoted originals from the local tier only | `Services/AdminOrderService.cs:168` | verified | `1e7b9d3` |
| PPW-150 | 🟠 | v1 | Cleanup job deletes Cloud uploads against the local tier and never deletes the large preview | `BackgroundJobs/UploadCleanupJob.cs:67` | verified | `1e7b9d3` |
| PPW-151 | 🟠 | v1 | Missing cloud original throws `AmazonS3Exception`, not `FileNotFoundException` → preview 500 | `Services/S3StorageService.cs:91` | verified | `1e7b9d3` |
| PPW-152 | 🟠 | v1 | Purge on Shipped fires once and skips an in-flight promotion → original never purged until reboot | `Services/AdminOrderService.cs:136` | verified | `1e7b9d3` |
| PPW-153 | 🟠 | v1 | Presigned-URL lifetime and the hardcoded `Cache-Control` max-age diverge → expired images | `Controllers/UploadsController.cs:185` | verified | `1e7b9d3` |
| PPW-154 | 🟠 | v1 (F5 part b) | Lightbox large URL is minted at list fetch and expires after its 1h lifetime, with no refresh | `UI/…/order-detail-page.ts` | verified | `972a8b4` |
| PPW-155 | 🟠 | v1 | Promotion worker disposes the concurrency semaphore while tasks are still in flight | `BackgroundJobs/OrderPhotoPromotionWorker.cs:108` | verified | `1e7b9d3` |
| PPW-156 | 🟠 | v1 | Migration dropping the `FilePath` NOT NULL constraint is unverified by any test | `Migrations/…MakeUploadFilePathNullable.cs` | verified | `1e7b9d3` |
| PPW-157 | 🟡 | v1 | Preview read races promotion deleting the local thumbnail → 500 instead of 404 | `Controllers/UploadsController.cs:190` | verified | `1e7b9d3` |
| PPW-158 | 🟡 | v1 | Duplicate payment webhooks race `Order.Status`; there is no concurrency token | `Controllers/WebhooksController.cs:218` | deferred | `2d02b13` |
| PPW-159 | 🟡 | v1 | 403 rather than 404 for another user's order tells an attacker which order ids exist | `Services/OrderService.cs:468` | wont-fix | `ac97e42` |
| PPW-160 | 🟡 | v1 | `/photos` returns presigned URLs with no `Cache-Control: private` | `Controllers/OrdersController.cs:82` | verified | `1e7b9d3` |
| PPW-161 | 🟡 | v1 | Guest-placed orders cannot reach the new `/photos` endpoint | `Controllers/OrdersController.cs:10` | wont-fix | `1e7b9d3` |
| PPW-162 | 🟠 | v1 | Empty-state copy collapses four causes into one permanent "no longer available", with no retry | `UI/…/order-detail-page.ts:103` | verified | `972a8b4` |
| PPW-163 | 🟡 | v1 | Cloud preview regeneration branch never runs — every test presets the thumbnail path | `Tests/…/CloudPreviewIntegrationTests.cs:225` | verified | `2d02b13` |
| PPW-164 | 🟡 | v1 | `GET /orders/{id}/photos` has no integration test, so its auth pipeline is unproven | `Controllers/OrdersController.cs:73` | verified | `1e7b9d3` |
| PPW-165 | 🟡 | v1 | Promoter row-update-failure and preview-generation-failure branches untested | `Services/OrderPhotoPromoter.cs:198` | verified | `1e7b9d3` |
| PPW-166 | 🟡 | v1 | Paid-then-cancelled originals: purge behaviour undecided and untested | `BackgroundJobs/OriginalPurgeRecoveryScanner.cs:54` | verified | `2d02b13` |
| PPW-167 | 🟡 | v1 | `BackfillCommand` and `S3BucketVerifier` have no tests at all | `Cli/BackfillCommand.cs:42` | verified | `1e7b9d3` |
| PPW-168 | 🟠 | v3 | Promotion recovery left boot-only while its purge sibling was made periodic | `BackgroundJobs/PromotionRecoveryScanner.cs` | verified | `972a8b4` |
| PPW-169 | 🟡 | v3 | The `FilePath` NOT NULL drop is verified on SQLite only, never on Postgres | `Tests/…/UploadThumbnailPathMigrationTests.cs:48` | deferred | `ac97e42` |
| PPW-170 | 🟠 | v3 | Purge sweep untested — tests call the sweep by reflection and `ExecuteAsync` is never driven | `BackgroundJobs/OriginalPurgeRecoveryScanner.cs:60` | verified | `972a8b4` |
| PPW-171 | 🟠 | v3 | Cancel-purge try/catch untested — a throwing purger is never exercised | `Services/AdminOrderService.cs:235` | verified | `972a8b4` |
| PPW-172 | 🟠 | v3 | Production-complete purge lacks the cancel path's try/catch → 500 after the transition committed | `Services/AdminOrderService.cs:135` | verified | `972a8b4` |
| PPW-173 | 🟠 | v3 | Tier resolve throws with cloud disabled and wedges every cleanup batch | `BackgroundJobs/UploadCleanupJob.cs:92` | verified | `972a8b4` |
| PPW-174 | 🟡 | v3 | ZIP tier resolve throws mid-stream with cloud disabled → truncated admin ZIP | `Services/AdminOrderService.cs:171` | verified | `972a8b4` |
| PPW-175 | 🟡 | v3 | Cleanup routes by the row's tier, so a failed promotion's cloud litter is never reclaimed | `BackgroundJobs/UploadCleanupJob.cs:92` | deferred | `972a8b4` |
| PPW-176 | 🟡 | v3 | Duplicate concurrent promotion re-creates a just-purged cloud original as an orphan | `Services/OrderPhotoPromoter.cs:168` | deferred | `2d02b13` |
| PPW-177 | 🟡 | v3 | New sweep-interval validator untested → an interval of 0 boots, then crashes the host | `Configuration/ArchiveSettings.cs:86` | verified | `972a8b4` |
| PPW-178 | 🟡 | v3 | Preview re-resolve-to-local success branch untested | `Controllers/UploadsController.cs:200` | verified | `972a8b4` |
| PPW-179 | 🟡 | v3 | Backfill filter test never crosses the exclusion boundary | `Tests/…/BackfillCommandTests.cs:40` | verified | `972a8b4` |
| PPW-180 | 🟠 | v3 | Upload thumbnail mints an unrevoked blob URL on every change-detection cycle | `UI/…/photo-thumbnail.component.ts:86` | verified | `972a8b4` |
| PPW-181 | 🟡 | v3 | Order detail redirects on any fetch error, so a transient failure bounces the user with no retry | `UI/…/order-detail-page.ts:357` | verified | `972a8b4` |
| PPW-182 | 🟡 | v3 | Lightbox modal has no focus trap, dialog role, modal flag or focus restore | `UI/…/photo-lightbox.component.ts` | verified | `972a8b4` |
| PPW-183 | 🟡 | v3 | Order detail loads only in `ngOnInit` despite a route-bound id → latent staleness | `UI/…/order-detail-page.ts:357` | deferred | `972a8b4` |
| PPW-184 | 🟡 | v4 (NF1) | Periodic promotion sweep has no in-flight dedup, so it can start a second promotion of one order | `BackgroundJobs/PromotionRecoveryScanner.cs` | deferred | `ac97e42` |
| PPW-185 | 🟠 | v5 | A stale lightbox photo id re-opens a closed lightbox when a thumbnail URL expires | `UI/…/order-detail-page.ts` | verified | `2d02b13` |
| PPW-186 | 🟠 | v5 | The periodic re-scan is untested and untestable — the interval has no test seam | `BackgroundJobs/PromotionRecoveryScanner.cs` | deferred | `ac97e42` |
| PPW-187 | 🟠 | v5 | Unroutable Cloud rows are skipped after the fetch, starving local-orphan cleanup | `BackgroundJobs/UploadCleanupJob.cs` | verified | `2d02b13` |
| PPW-188 | 🟡 | v5 | Renamed guard tests seed an empty database, so they pass for the wrong reason | `Tests/…/PromotionRecoveryScannerTests.cs` | backlog | `2d02b13` |
| PPW-189 | 🟡 | v5 | The anti-refresh-loop guard has no test | `UI/…/order-detail-page.ts` | backlog | `2d02b13` |
| PPW-190 | 🟡 | v5 | The lightbox focus trap has no spec | `UI/…/photo-lightbox.component.ts` | backlog | `ac97e42` |
| PPW-191 | 🟡 | v5 | The lightbox tells the user to reload while the app is already fetching a fresh URL | `UI/…/photo-lightbox.component.ts` | backlog | `ac97e42` |
| PPW-192 | 🟡 | v5 | A 401 for a non-authenticated user leaves a blank order body, with no error and no redirect | `UI/…/order-detail-page.ts` | backlog | `2d02b13` |
| PPW-193 | ⚪ | v5 | Order retries and init subscriptions have no in-flight dedup or teardown | `UI/…/order-detail-page.ts` | backlog | `2d02b13` |
| PPW-194 | 🟡 | v5 | The ZIP pre-flight throws an unmapped exception → a generic 500 logged as unhandled | `Services/AdminOrderService.cs` | backlog | `ac97e42` |
| PPW-195 | 🟡 | v5 | The periodic sweep re-enqueues permanently failed promotions forever | `BackgroundJobs/PromotionRecoveryScanner.cs` | deferred | `ac97e42` |
| PPW-196 | 🟡 | v5 | The cloud-enabled flag is fixed at boot, so switching provider at runtime needs a restart | `BackgroundJobs/PromotionRecoveryScanner.cs` | deferred | `ac97e42` |
| PPW-197 | 🟡 | v5 | The lightbox failure flag is reset only on a changed URL, so an identical refreshed URL stays stuck | `UI/…/photo-lightbox.component.ts` | backlog | `ac97e42` |
| PPW-198 | 🔴 | v7 | The S3 upload rewinds the stream outside the retry loop → a retried upload silently loses the photo | `Services/S3StorageService.cs:63` | verified | `ac97e42` |
| PPW-199 | 🟠 | v7 | Purge and retention destroy a photo a second still-active order needs | `Services/OriginalPurger.cs:103` | verified | `ac97e42` |
| PPW-200 | 🟠 | v7 | The promotion worker holds its concurrency slot through the whole retry backoff | `BackgroundJobs/OrderPhotoPromotionWorker.cs:107` | verified | `ac97e42` |
| PPW-201 | 🟠 | v7 | The photos query has no soft-delete filter → presigned URLs for uploads whose blobs are gone | `Services/OrderService.cs:460` | verified | `ac97e42` |
| PPW-202 | 🟠 | v7 | The webhook paid transition is an unguarded check-then-act → duplicate confirmation emails | `Controllers/WebhooksController.cs:215` | deferred | `ac97e42` |
| PPW-203 | 🟠 | v7 | Upload service re-advertises HEIC while the validator and the UI still reject it | `Services/UploadService.cs:52` | verified | `ac97e42` |
| PPW-204 | 🟠 | v7 | Client filename is not truncated to the column width → Postgres rejects it and returns 500 | `Services/UploadService.cs:113` | verified | `ac97e42` |
| PPW-205 | 🟠 | v7 | The retention audit event is written before the save, so a failed save leaves false audit records | `BackgroundJobs/ArchiveRetentionJob.cs:123` | verified | `ac97e42` |
| PPW-206 | 🟠 | v7 | Cloud-off purge refusal is logged at Error on every ship in the default configuration | `Services/OriginalPurger.cs:43` | verified | `ac97e42` |
| PPW-207 | 🟠 | v7 | The promotion worker's retry, backoff and re-enqueue path is entirely untested | `BackgroundJobs/OrderPhotoPromotionWorker.cs:130` | verified | `ac97e42` |
| PPW-208 | 🟠 | v7 | No test asserts that the payment webhook enqueues promotion | `Controllers/WebhooksController.cs:183` | verified | `ac97e42` |
| PPW-209 | 🟠 | v7 | A real cloud provider is never exercised — only a skip-gated MinIO suite and fakes | `Tests/…/S3StorageServiceIntegrationTests.cs:18` | deferred | `ac97e42` |
| PPW-210 | 🟡 | v7 | Retention's fixed candidate window is starved by rows whose delete keeps failing | `BackgroundJobs/ArchiveRetentionJob.cs:98` | backlog | `ac97e42` |
| PPW-211 | 🟡 | v7 | A read failure part-way through the admin ZIP truncates the archive after the headers are sent | `Services/AdminOrderService.cs:197` | backlog | `ac97e42` |
| PPW-212 | 🟡 | v7 | Preview cache-fill regeneration races the retention delete → an orphaned blob and a null reference | `Services/UploadService.cs:203` | backlog | `ac97e42` |
| PPW-213 | 🟡 | v7 | A failed best-effort local delete in the promoter leaks local bytes nothing reclaims | `Services/OrderPhotoPromoter.cs:212` | backlog | `ac97e42` |
| PPW-214 | 🟡 | v7 | The local storage root re-anchor uses a prefix match with no separator boundary | `Services/LocalStorageService.cs:99` | backlog | `ac97e42` |
| PPW-215 | 🟡 | v7 | The ZIP entry extension is taken from the untrusted client filename, not the validated type | `Services/AdminOrderService.cs:190` | backlog | `ac97e42` |
| PPW-216 | 🟡 | v7 | Batch upload caps total bytes but not the number of files | `Controllers/UploadsController.cs:102` | backlog | `ac97e42` |
| PPW-217 | 🟡 | v7 | A broken grid thumbnail has no fallback or retry after the one presigned-URL refresh | `UI/…/order-detail-page.ts:472` | backlog | `ac97e42` |
| PPW-218 | 🟡 | v7 | Originals of orders that never reach production-complete or Cancelled escape the retention window | `BackgroundJobs/ArchiveRetentionJob.cs:92` | backlog | `ac97e42` |
| PPW-219 | 🟡 | v7 | The documented 502 for a persistent storage failure is not implemented; it surfaces as 500 | `Services/S3StorageService.cs:145` | backlog | `ac97e42` |
| PPW-220 | 🟡 | v7 | Idempotent-skip reasons are logged at Debug and never emit under the Information floor | `Services/OrderPhotoPromoter.cs:120` | backlog | `ac97e42` |
| PPW-221 | 🟡 | v7 | Transient and permanent cloud-write failures collapse into one warning, so poison is retried | `Services/OrderPhotoPromoter.cs:182` | backlog | `ac97e42` |
| PPW-222 | 🟡 | v7 | The preview cache-hit path lost its no-tracking read | `Services/UploadService.cs:139` | backlog | `ac97e42` |
| PPW-223 | 🟡 | v7 | The promotable-status set is written out three times under a false single-source comment | `Cli/BackfillCommand.cs:43` | backlog | `ac97e42` |
| PPW-224 | 🟡 | v7 | The S3 retry classification, re-upload and presign protocol are untested | `Services/S3StorageService.cs:60` | backlog | `ac97e42` |
| PPW-225 | 🟡 | v7 | Storage wiring, configuration and the CLI sat outside the lens list; the region setting is a trap | `Extensions/StorageExtensions.cs:56` | backlog | `ac97e42` |
| PPW-226 | 🟡 | v7 | Recovery and retention sweeps run unindexed full scans every six hours | `Data/Configurations/UploadConfiguration.cs:30` | backlog | `ac97e42` |
| PPW-227 | ⚪ | v7 | The promoter reads the whole original into an array and leaves memory streams undisposed | `Services/OrderPhotoPromoter.cs:138` | backlog | `ac97e42` |
| PPW-228 | ⚪ | v7 | The best-effort orphan-thumbnail delete swallows its exception with no log | `Services/UploadService.cs:222` | backlog | `ac97e42` |
| PPW-229 | ⚪ | v7 | Local preview cache header disagrees with the documented one | `Controllers/UploadsController.cs:26` | backlog | `ac97e42` |
...
| PPW-237 | 🟡 | v9 | Promoter tests assert the cloud keys written but never the bytes | `Tests/…/OrderPhotoPromoterTests.cs` | backlog | `ac97e42` |
| PPW-238 | ⚪ | v9 | Code comments cite finding, decision and design-record ids, which the repo rule bans | codebase-wide | fixed | `09173c4` |
| PPW-239 | 🟡 | v9 | Closing the lightbox during a refresh has no spec; only closing before the error is tested | `UI/…/order-detail-page.spec.ts` | backlog | `ac97e42` |

## Details

### PPW-149 — Admin ZIP fulfilment download reads promoted originals from the local tier only
...
- **History:**
  - v1: found — the pass's only High, and the one fix it required before merge
  - round 1: fixed @`ec94fca` — the ZIP reads through the storage router
  - v2: verified @`1e7b9d3`

### PPW-150 — Cleanup job deletes Cloud uploads against the local tier and never deletes the large preview
...
### PPW-151 — Missing cloud original throws `AmazonS3Exception`, not `FileNotFoundException` → preview 500
...
### PPW-152 — Purge on Shipped fires once and skips an in-flight promotion → original never purged until reboot
...
### PPW-153 — Presigned-URL lifetime and the hardcoded `Cache-Control` max-age diverge → expired images
...
- **What:** The preview redirect hardcoded a one-hour cache lifetime while the presigned URL lifetime
  ...
  already-expired URL.
- **History:**
  - v1: found — part b split off as PPW-154
  - round 1: fixed @`d15b9af` — the cache lifetime is derived from the presign setting
  - v2: verified @`1e7b9d3`

### PPW-154 — Lightbox large URL is minted at list fetch and expires after its 1h lifetime, with no refresh
...
- **History:**
  ...
  - v3: re-found and confirmed with a trace
...
### PPW-155 — Promotion worker disposes the concurrency semaphore while tasks are still in flight
...
### PPW-156 — Migration dropping the `FilePath` NOT NULL constraint is unverified by any test
...
- **History:**
  - v1: found — hinted by the shared dual-database context
...
### PPW-157 — Preview read races promotion deleting the local thumbnail → 500 instead of 404
...
### PPW-158 — Duplicate payment webhooks race `Order.Status`; there is no concurrency token
...
- **History:**
  ...
  - v7: re-raised (pass A), decision upheld @`2d02b13`; PPW-202 adds the duplicate-email consequence to the same remit
  - 2026-07-22: target closed with the row still deferred

### PPW-159 — 403 rather than 404 for another user's order tells an attacker which order ids exist
...
### PPW-160 — `/photos` returns presigned URLs with no `Cache-Control: private`
...
- **History:**
  - v1: found — judged plausible, not proven live
  - round 1: fixed @`751894b` — the response sets a private, no-store cache header
  - v2: verified @`1e7b9d3`

### PPW-161 — Guest-placed orders cannot reach the new `/photos` endpoint
...
### PPW-162 — Empty-state copy collapses four causes into one permanent "no longer available", with no retry
...
- **History:**
  - v1: found — 🟡, scoped to the not-yet-archived case
  - round 1: deferred to the frontend lens the lean pass skipped
  - v2: decision upheld
  - v3: re-found and widened by that lens, re-rated 🟠
  - round 3: fixed @`c4ec6ca` — a fetch failure is separated from a genuine empty result and offers a retry
  - v4: verified @`972a8b4`; the four-way empty signal stays an open follow-up, later raised as PPW-232

### PPW-163 — Cloud preview regeneration branch never runs — every test presets the thumbnail path
...
### PPW-164 — `GET /orders/{id}/photos` has no integration test, so its auth pipeline is unproven
...
### PPW-165 — Promoter row-update-failure and preview-generation-failure branches untested
...
### PPW-166 — Paid-then-cancelled originals: purge behaviour undecided and untested
...
### PPW-167 — `BackfillCommand` and `S3BucketVerifier` have no tests at all
...
### PPW-168 — Promotion recovery left boot-only while its purge sibling was made periodic

- **What:** The PPW-152 fix made the purge scanner periodic but left the promotion scanner running once at
...
- **History:**
  ...
  - v3: found and confirmed with a trace
...
### PPW-169 — The `FilePath` NOT NULL drop is verified on SQLite only, never on Postgres
...
- **History:**
  - v3: found — hinted by the shared dual-database context
...
### PPW-170 — Purge sweep untested — tests call the sweep by reflection and `ExecuteAsync` is never driven

- **What:** The PPW-152 conversion left the tests calling the internal sweep method by reflection, and both
  entry-point tests returned at a guard. Deleting the boot sweep or the timer loop left the suite green.
- **History:**
  - v3: found — a coverage regression introduced by the PPW-152 fix
  - round 3: fixed @`fea2490` — a test drives the entry point and reddens when the boot sweep is removed
  - v4: verified @`972a8b4`

### PPW-171 — Cancel-purge try/catch untested — a throwing purger is never exercised

- **What:** The guard added for PPW-166 kept a purge failure from failing an already-committed cancellation,
...
### PPW-172 — Production-complete purge lacks the cancel path's try/catch → 500 after the transition committed

- **What:** The PPW-166 fix wrapped the cancel purge but not its production-complete sibling. A purge that
  threw there returned 500 to the admin after the status change had already been committed, emailed and
  broadcast.
- **History:**
  - v3: found — the PPW-166 fix treated the instance, not the class
  - round 3: fixed @`c30d734` — the same guard on the production-complete purge
  - v4: verified @`972a8b4`

### PPW-173 — Tier resolve throws with cloud disabled and wedges every cleanup batch

- **What:** The PPW-150 routing fix resolved the tier outside the per-upload guard. With cloud switched off
...
- **History:**
  - v3: found — a new fault introduced by the PPW-150 fix
...
### PPW-174 — ZIP tier resolve throws mid-stream with cloud disabled → truncated admin ZIP
...
- **History:**
  - v3: found — the same class as PPW-173
  - round 3: fixed @`c30d734`, `0fc577a` — the ZIP fails before writing any response bytes
  - v4: verified @`972a8b4`

### PPW-175 — Cleanup routes by the row's tier, so a failed promotion's cloud litter is never reclaimed
...
- **History:**
  - v3: found — a residual of the PPW-150 routing fix
...
### PPW-176 — Duplicate concurrent promotion re-creates a just-purged cloud original as an orphan
...
- **Suggested fix:** Re-read the live tier and path before the flip, or put a concurrency token on the
  upload, in the same work that owns PPW-158.
- **History:**
  - v3: found — the new and more frequent purge triggers widened the window
  - round 3: deferred → bolt-035, with PPW-158
  - v4: deferral upheld @`972a8b4`, but the rationale was judged incomplete: it rests on duplicate webhooks, and the new sweep is a second trigger. Raised as PPW-184
  - v7: re-raised (pass A), decision upheld @`2d02b13`
  - 2026-07-22: target closed with the row still deferred

### PPW-177 — New sweep-interval validator untested → an interval of 0 boots, then crashes the host

- **What:** The validator rule added with the PPW-152 fix had no test. Dropping it let an interval of zero
...
### PPW-178 — Preview re-resolve-to-local success branch untested

- **What:** The PPW-157 fix's re-resolve had tests for the redirect and the double-miss, but not for the
  case where the second local read succeeds. A regression there would have reddened nothing.
- **History:**
  - v3: found — a coverage gap, no live defect
  - round 3: fixed @`66a5f64` — a test drives the successful re-resolve
  - v4: verified @`972a8b4`

### PPW-179 — Backfill filter test never crosses the exclusion boundary

- **What:** The PPW-167 test claimed to guard against filter drift but seeded only included statuses, so
  widening the filter to re-promote cancelled or refunded photos would have shipped green.
- **History:**
  - v3: found — a residual of the PPW-167 fix
  - round 3: fixed @`66a5f64` — boundary cases on both sides of the filter
  - v4: verified @`972a8b4`

### PPW-180 — Upload thumbnail mints an unrevoked blob URL on every change-detection cycle
...
### PPW-181 — Order detail redirects on any fetch error, so a transient failure bounces the user with no retry
...
- **History:**
  - v3: found — the Medium strand refuted, the Low residual kept
...
### PPW-182 — Lightbox modal has no focus trap, dialog role, modal flag or focus restore
...
- **History:**
  - v3: found — first accessibility coverage of this surface
  - round 3: fixed @`a5cb0be` — dialog role, modal flag, focus move, tab trap and focus restore
  - v4: verified @`972a8b4`

### PPW-183 — Order detail loads only in `ngOnInit` despite a route-bound id → latent staleness
...
- **History:**
  - v3: found — latent, not triggerable today
  - round 3: deferred as a recorded trap
  - v4: deferral upheld @`972a8b4`
  - 2026-07-22: target closed with the row still deferred

### PPW-184 — Periodic promotion sweep has no in-flight dedup, so it can start a second promotion of one order
...
- **What:** The sweep queues every paid order still holding a local upload, with no check against jobs
  ...
  promotion starts a second one, which reaches the PPW-176 orphan outcome without needing duplicate webhooks.
...
- **Suggested fix:** Fold into the concurrency-token work that owns PPW-158 and PPW-176; de-duplicate the queue by
  order.
- **History:**
  - v4: found (NF1) while checking the PPW-176 deferral rationale — introduced by the PPW-168 fix
  - v5: independently re-found by the blinded race lens, with the false failure signal as a new consequence
...
### PPW-185 — A stale lightbox photo id re-opens a closed lightbox when a thumbnail URL expires
...
- **History:**
  - v5: found — a regression from the PPW-154 refresh fix
...
### PPW-186 — The periodic re-scan is untested and untestable — the interval has no test seam

- **What:** The only test awaits the boot sweep, so deleting the periodic loop leaves the suite green and
  restores the exact PPW-168 defect. The interval is configured in whole hours, so no fast periodic test can
  be written.
...
- **History:**
  - v5: found — the PPW-168 fix shipped its headline behaviour untested
...
### PPW-187 — Unroutable Cloud rows are skipped after the fetch, starving local-orphan cleanup
...
- **History:**
  - v5: found — the edge the PPW-173 fix dismissed as out of scope
...
### PPW-188 — Renamed guard tests seed an empty database, so they pass for the wrong reason
...
- **Suggested fix:** Add a spec that dispatches a second error and asserts the photos endpoint was called
  exactly twice.
- **History:**
  - v5: found — coverage left by the PPW-154 refresh fix
...
- **What:** The tab trap added for PPW-182 has no test. Dropping the prevent-default or the refocus lets tab
...
- **History:**
  - v5: found — coverage left by the PPW-182 fix
  - round 5: sent to backlog
  - v6: backlog upheld @`2d02b13`
...
- **History:**
  - v5: found — user experience left by the PPW-154 refresh fix
  - round 5: sent to backlog
  - v6: backlog upheld @`2d02b13`
...
- **History:**
  - v5: found — hinted by the shared guest-authentication context
  - round 5: sent to backlog
...
- **What:** With cloud off, the admin ZIP pre-flight added for PPW-174 throws an exception the middleware does
...
- **Suggested fix:** Throw a mapped domain error with a 409 or 422 status and log the cloud-off reason as
  a warning.
- **History:**
  - v5: found — a residual of the PPW-174 fix
...
- **History:**
  - v5: found — introduced by the PPW-168 conversion to a periodic sweep
  - round 5: deferred → bolt-035 as part of cluster A
  - v6: deferral upheld @`2d02b13`
  - v9: the steady-state cost and poison-amplification flag folded back here @`ac97e42`
  - 2026-07-22: target closed with the row still deferred

### PPW-196 — The cloud-enabled flag is fixed at boot, so switching provider at runtime needs a restart
...
- **History:**
  - v5: found — surfaced by the PPW-168 conversion
  - round 5: deferred → bolt-035 as part of cluster A
  - v6: deferral upheld @`2d02b13`
  - v8: file untouched in the round; deferral spot-checked @`ac97e42`
  - 2026-07-22: target closed with the row still deferred

### PPW-197 — The lightbox failure flag is reset only on a changed URL, so an identical refreshed URL stays stuck
...
- **History:**
  - v5: found — an edge of the PPW-154 and PPW-182 fixes, judged narrow
  - round 5: sent to backlog
  - v6: backlog upheld @`2d02b13`
  - v8: file untouched in the round; backlog spot-checked @`ac97e42`
  - 2026-08-10: row carried to the backlog
...
### PPW-199 — Purge and retention destroy a photo a second still-active order needs
...
### PPW-200 — The promotion worker holds its concurrency slot through the whole retry backoff
...
- **History:**
  ...
  - v9: the cost and poison-amplification flag noted against this row and PPW-195

### PPW-201 — The photos query has no soft-delete filter → presigned URLs for uploads whose blobs are gone
...
### PPW-202 — The webhook paid transition is an unguarded check-then-act → duplicate confirmation emails

- **What:** Concurrent duplicate payment deliveries both pass the status check, so promotion is queued
  twice and the customer receives two confirmation emails. Same root cause as PPW-158, with the email
  consequence added.
...
- **History:**
  - v7: found (pass B, F5)
  - round 7: deferred → bolt-035, with PPW-158
  - v8: deferral upheld @`ac97e42` — the controller was untouched in the round
  - 2026-07-22: target closed with the row still deferred

### PPW-203 — Upload service re-advertises HEIC while the validator and the UI still reject it
...
### PPW-204 — Client filename is not truncated to the column width → Postgres rejects it and returns 500
...
### PPW-205 — The retention audit event is written before the save, so a failed save leaves false audit records
...
### PPW-206 — Cloud-off purge refusal is logged at Error on every ship in the default configuration
...
### PPW-207 — The promotion worker's retry, backoff and re-enqueue path is entirely untested
...
- **History:**
  ...
  - v8: verified @`ac97e42` — by construction through the PPW-200 revert

### PPW-208 — No test asserts that the payment webhook enqueues promotion
...
### PPW-209 — A real cloud provider is never exercised — only a skip-gated MinIO suite and fakes

- **What:** Cloud behaviour is proven only against a MinIO container in continuous integration and
  in-memory fakes. No test ever runs against the real provider, including the retry path PPW-198 lived in.
...
- **History:**
  - v7: found (pass B, F12)
  - round 7: deferred → the three-environment track, with PPW-169; the PPW-198 regression tests shrank what only a real provider can prove
  - v8: deferral upheld @`ac97e42` — the integration tests were untouched in the round
  - 2026-07-22: target closed with the row still deferred

### PPW-210 — Retention's fixed candidate window is starved by rows whose delete keeps failing
...
- **Evidence:** `BackgroundJobs/ArchiveRetentionJob.cs:98`. The same shape as PPW-187, in a second job.
...
- **What:** Nothing tested which failures the adapter treats as retryable, what a retried upload sends, or
  which protocol the signed URL uses. This is the coverage gap PPW-198 lived inside.
...
- **History:**
  ...
  - 2026-07-27: owner ruled defer to the three-environment stage, with PPW-169 and PPW-209; affirmed @`d041295`
  - 2026-07-22: target closed with the row still deferred

### PPW-235 — Retention deletes the blobs before it persists the null keys → a broken-URL window
...
- **Evidence:** `BackgroundJobs/ArchiveRetentionJob.cs:96`. The same class as PPW-201 and PPW-205, missed when
  those were fixed.
...
### PPW-239 — Closing the lightbox during a refresh has no spec; only closing before the error is tested

- **What:** The PPW-185 fix also covers closing the lightbox while the refresh is still in flight, by re-reading
