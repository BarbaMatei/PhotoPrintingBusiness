...
|---|---|---|---|---|---|---|
| PPW-1 | 🔴 | v1 | Two concurrent requests with the same key both insert, and the loser returns 500 instead of a replay | `Services/OrderService.cs:392` | verified | `650f615` |
| PPW-2 | 🟠 | v1 | The idempotency lookup is not scoped to the caller, so another tenant's order and its Stripe secret can be handed over | `Services/OrderService.cs:437` | verified | `650f615` |
| PPW-3 | 🟠 | v1 | The divergence check ignores cart contents, so a reused key can replay the wrong photos at the same total | `Services/OrderService.cs:449` | verified | `650f615` |
| PPW-4 | 🟠 | v1 | Nothing tracks the planned change that makes the key header required | `Controllers/PaymentsController.cs:105` | verified | `650f615` |
| PPW-5 | 🟠 | v1 | A second database round-trip looks up the stale row the first query already covered | `Services/OrderService.cs:408` | verified | `650f615` |
| PPW-6 | 🟠 | v1 | Header extraction and the missing-key warning are copied into both payment endpoints | `Controllers/PaymentsController.cs:49` | verified | `650f615` |
| PPW-7 | 🟠 | v1 | The replay, compute and persist sequence is duplicated between the two processor branches | `Controllers/PaymentsController.cs:51` | verified | `650f615` |
| PPW-8 | 🟡 | v1 | A freed and reused key is forwarded to Stripe with a possibly different amount | `Services/StripePaymentGateway.cs:31` | verified | `650f615` |
| PPW-9 | 🟡 | v1 | The migration writes an unfiltered `TEXT` index while the runtime model filters it on Postgres | `Migrations/20260527075359_AddOrderIdempotencyKey.cs:34` | verified | `650f615` |
| PPW-10 | 🟡 | v1 | A comment credits the per-statement constraint check to Postgres alone, though SQLite enforces it too | `Services/OrderService.cs:405` | verified | `650f615` |
| PPW-11 | 🟡 | v1 | The unique-index null comment reads as if duplicate nulls were forbidden | `Data/PhotoPrintDbContext.cs:146` | verified | `650f615` |
| PPW-12 | 🟡 | v1 | The ddd-02 design sketch puts conflict resolution in the controller, the code puts it in the order service | `memory-bank/…/ddd-02` | backlog | `650f615` |
| PPW-13 | ⚪ | v1 | A second conflict exception type exists only to carry the divergent-field payload | `Exceptions/IdempotencyConflictException.cs` | wont-fix | `b52f4b6` |
| PPW-14 | ⚪ | v1 | The correlation id is read out of the request items bag by a raw string key in two places | `Controllers/PaymentsController.cs:109` | verified | `650f615` |
| PPW-15 | ⚪ | v1 | Each create saves twice, once in the service and once in the controller | `Controllers/PaymentsController.cs:65` | wont-fix | `b52f4b6` |
| PPW-16 | ⚪ | v2 | The cross-tenant integration test runs on a provider that does not enforce the unique index | `Tests/…/PaymentControllerIntegrationTests.cs:79` | verified | `650f615` |
...
| PPW-24 | 🟡 | v5 | With both owner ids null the scope test collapses to "any order without a guest id" | `Services/OrderService.cs:235` | verified | `3faaae6` |
| PPW-25 | 🟡 | v5 | Key length is never checked, so an over-long key fails on Postgres instead of being refused | `Filters/IdempotencyKeyFilter.cs:22` | verified | `3faaae6` |
| PPW-26 | 🟡 | v5 | The reserved conflict log event is never emitted | `Middleware/ExceptionHandlerMiddleware.cs` | verified | `3faaae6` |
| PPW-27 | 🟡 | v5 | The recovery replay calls the gateway again and writes no replay log, so it reads as a fresh request | `Controllers/PaymentsController.cs:117` | verified | `3faaae6` |
| PPW-28 | 🟡 | v5 | The design document says the stale key is freed inside the insert's transaction; the code uses a separate save | `Services/OrderService.cs:108` | verified | `3faaae6` |
| PPW-29 | ⚪ | v5 | No document states that the gateway is keyed by the order id rather than the caller's key | `memory-bank/…/ddd-02` | verified | `3faaae6` |
| PPW-30 | ⚪ | v5 | The pre-insert and post-collision resolution blocks are near duplicates | `Services/OrderService.cs` | verified | `3faaae6` |
| PPW-31 | ⚪ | v5 | Provider names are written out as literal strings in four places | `Data/PhotoPrintDbContext.cs` | verified | `3faaae6` |
| PPW-32 | ⚪ | v5 | The controller saves through the database context itself rather than through the order service | `Controllers/PaymentsController.cs:125` | backlog | `fbb4c7c` |
...
| PPW-40 | 🟡 | v8 | The legacy processor recovery replay builds a fresh redirect URL instead of returning the stored one | `Controllers/PaymentsController.cs:131` | backlog | `065a516` |
| PPW-41 | 🟡 | v8 | The key is never trimmed, so a padded copy of the same key creates a second order and a second charge | `Filters/IdempotencyKeyFilter.cs:23` | verified | `01b5264` |
| PPW-42 | 🟡 | v8 | Freeing the stale key and inserting the new order are two separate saves with nothing to roll them back together | `Services/OrderService.cs:99` | verified | `01b5264` |
| PPW-43 | 🟡 | v8 | On SQLite two racing requests can collide on the order-number index first, which the recovery does not handle | `Services/OrderService.cs:162` | verified | `01b5264` |
| PPW-44 | 🟡 | v8 | The order service's price-tier lookup is a comment-claimed copy of the cart service's, but reads a different quantity | `Services/OrderService.cs:413` | verified | `01b5264` |
| PPW-45 | ⚪ | v8 | The public key lookup has no production caller left | `Services/OrderService.cs:220` | verified | `01b5264` |
| PPW-46 | ⚪ | v8 | The cart seed graph is rebuilt in three test fixtures and has already drifted apart | `Tests/…/OrderServiceTests.cs:43` | verified | `01b5264` |
| PPW-47 | ⚪ | v8 | The concurrency test hand-builds the winning order with copied number totals | `Tests/…/OrderServiceIdempotencyConcurrencyTests.cs:166` | verified | `01b5264` |
| PPW-48 | ⚪ | v8 | The two replay-logging branches repeat the event shape and test the replay flag twice | `Controllers/PaymentsController.cs:115` | verified | `01b5264` |
| PPW-49 | ⚪ | v8 | The exception middleware reads the hosting environment two different ways in one request | `Middleware/ExceptionHandlerMiddleware.cs:88` | verified | `01b5264` |
| PPW-50 | ⚪ | v8 | The declared 409 response has no body type, so generated clients never see the divergent-field list | `Controllers/PaymentsController.cs:45` | verified | `01b5264` |
| PPW-51 | ⚪ | v8 | The transitional missing-key event logs at warning level on every payment request | `Filters/IdempotencyKeyFilter.cs:49` | verified | `065a516` |

## Details

### PPW-1 — Two concurrent requests with the same key both insert, and the loser returns 500 instead of a replay
...
- **History:**
  - v1: found — one of the pass's two fixes required before merge; two lenses reached it
  ...
  - v5: the catch's breadth was raised separately as PPW-23; v8 raised the SQLite match as PPW-38

### PPW-2 — The idempotency lookup is not scoped to the caller, so another tenant's order and its Stripe secret can be handed over
...
- **History:**
  - v1: found — the second of the pass's two fixes required before merge; three lenses reached it
  ...
  - v3: re-checked, unchanged @`b6198b6` · v4: re-checked, unchanged @`650f615`
  - v5: the both-null edge of the same predicate was raised separately as PPW-24

### PPW-3 — The divergence check ignores cart contents, so a reused key can replay the wrong photos at the same total
...
- **What:** Divergence compared four scalar fields but not the items. With uniform per-print pricing,
  ...
  images.
- **History:**
  - v1: found — strongly recommended alongside the pass's two required fixes
...
### PPW-4 — Nothing tracks the planned change that makes the key header required
...
- **History:**
  ...
  - v2: verified @`b52f4b6`
  - v3: the PPW-6 refactor dropped the marker, recorded separately as PPW-19; the warning behaviour was unchanged
  - v4: re-checked after PPW-19's fix restored the marker @`650f615`

### PPW-5 — A second database round-trip looks up the stale row the first query already covered
...
### PPW-6 — Header extraction and the missing-key warning are copied into both payment endpoints
...
- **History:**
  - v1: found — deferred at v2 as a cross-cutting refactor, then implemented on request
...
### PPW-7 — The replay, compute and persist sequence is duplicated between the two processor branches

- **What:** The Stripe and the legacy processor branches had the same shape and differed only in the gateway call
  and the cached field.
- **History:**
  - v1: found — deferred at v2, then implemented on request
...
### PPW-8 — A freed and reused key is forwarded to Stripe with a possibly different amount
...
### PPW-9 — The migration writes an unfiltered `TEXT` index while the runtime model filters it on Postgres
...
- **History:**
  ...
  - v3: the fix left the snapshot half undone; that residual was raised at v5 as PPW-20
  - v4: re-checked, unchanged @`650f615`

### PPW-10 — A comment credits the per-statement constraint check to Postgres alone, though SQLite enforces it too
...
### PPW-11 — The unique-index null comment reads as if duplicate nulls were forbidden
...
- **History:**
  - v1: found — recorded as low severity and contested in the same paragraph
  - round 1: fixed @`b52f4b6` — reworded to "multiple nulls are permitted"
  - v2: verified @`b52f4b6` · v3: re-checked, unchanged @`b6198b6` · v4: re-checked, unchanged @`650f615`

### PPW-12 — The ddd-02 design sketch puts conflict resolution in the controller, the code puts it in the order service
...
- **History:**
  - v1: found
  - round 1: wont-fix — the PPW-1 fix also throws the plain conflict type, so both now carry meaning
  - v2: ruling accepted @`b52f4b6`

### PPW-14 — The correlation id is read out of the request items bag by a raw string key in two places
...
### PPW-15 — Each create saves twice, once in the service and once in the controller

- **What:** The order is saved before the gateway call and again after it.
- **History:**
  - v1: found — the review itself noted the split may be deliberate
  - round 1: wont-fix — the order must exist before the gateway call so a crash mid-call is recoverable
  - v2: ruling accepted @`b52f4b6`

### PPW-16 — The cross-tenant integration test runs on a provider that does not enforce the unique index
...
- **History:**
  - v2: found — informational, raised while verifying the PPW-2 fix
  ...
  - v3: the factory had to fake the order-number service, which exposed PPW-18
  - v4: re-checked and strengthened once the fake was removed @`650f615`

### PPW-17 — An expired key stays reserved for its first owner, so a second caller is refused a key the contract calls free
...
- **What:** Stale keys are freed only when their own owner resubmits. A different caller presenting that
  ...
  free after a day.
- **History:**
  - v2: found — recorded as an accepted consequence of keeping one global key index
  ...
  - v8: re-found, severity raised from ⚪ to 🟡 as a contract mismatch rather than an edge case
...
### PPW-18 — The order-number service has no SQLite branch, so every order creation in the development environment fails
...
- **What:** The service had a count branch for the in-memory provider and Postgres-only SQL for
  ...
  syntax and failed.
- **History:**
  - v3: found — surfaced because the PPW-16 test factory had to fake the service to run at all
  - round 1: fixed @`3415ec7` — SQLite joins the count branch; the Postgres path is byte-identical
  - v4: verified @`650f615` — reverting the branch reddens both new tests and flips the PPW-16 test from 409 to 500
  - v4: the count branch's behaviour under concurrency was noted and dismissed as not a finding; v8 raised it as PPW-43

### PPW-19 — The filter refactor dropped the searchable follow-up marker PPW-4 asked for

- **What:** Moving the warning into the new filter replaced the marker and its document pointer with
  prose, so the search that PPW-4 existed to answer returned nothing.
- **History:**
  - v3: found — introduced by the PPW-6 fix; the warning behaviour itself was unchanged
...
### PPW-20 — The model snapshot is SQLite-flavoured, so the next Postgres scaffold emits a phantom migration
...
- **Suggested fix:** Per-provider migration assemblies, or regenerate the snapshot under Postgres. A
  ...
  these columns.
- **History:**
  - v5: found — the residual left by PPW-9's fix, which corrected the migration but not the snapshot
...
- **History:**
  - v5: found — one of the two items the pass recommended before deploying
...
### PPW-22 — The 409 body names the divergent fields only outside Development, and no test reads the body
...
- **History:**
  - v5: found — the pass's second recommendation before deploying
...
### PPW-23 — The recovery catch infers that any database write failure was the key collision
...
- **History:**
  - v5: found — a residual of PPW-1's fix; distinct from PPW-1, which was the 500 itself
  ...
  - v8: the SQLite half of that constraint match was raised separately as PPW-38

### PPW-24 — With both owner ids null the scope test collapses to "any order without a guest id"
...
- **History:**
  - v5: found — a residual of PPW-2's fix, not reachable at the reviewed commit
...
### PPW-25 — Key length is never checked, so an over-long key fails on Postgres instead of being refused
...
### PPW-26 — The reserved conflict log event is never emitted
...
- **History:**
  ...
  - v8: the cross-tenant collision left outside that scope was raised as PPW-37

### PPW-27 — The recovery replay calls the gateway again and writes no replay log, so it reads as a fresh request
...
- **History:**
  ...
  - v8: the legacy processor half of the same path was raised as PPW-40

### PPW-28 — The design document says the stale key is freed inside the insert's transaction; the code uses a separate save
...
- **History:**
  ...
  - v6: verified @`3faaae6`
  - v8: the non-atomicity itself was raised as a defect, PPW-42, overturning this round's document-only ruling

### PPW-29 — No document states that the gateway is keyed by the order id rather than the caller's key

- **What:** The design recorded the signature change but never the choice PPW-8's fix made, so a future
...
- **History:**
  ...
  - v6: verified @`3faaae6`, with the incomplete half of the same edit recorded as PPW-35

### PPW-30 — The pre-insert and post-collision resolution blocks are near duplicates
...
### PPW-31 — Provider names are written out as literal strings in four places
...
### PPW-32 — The controller saves through the database context itself rather than through the order service

- **What:** The controller persists the gateway field with its own save, which sits below the layer the
  order service owns.
- **History:**
  - v5: found — the pass recorded it as a pre-existing pattern, not introduced by this work
...
### PPW-34 — The order-number query raises a compiler warning and its Postgres branch has no test
...
- **History:**
  ...
  - v6: verified @`3faaae6` — the warning is gone from the build; the coverage gap is unchanged and became part of PPW-36

### PPW-35 — The ddd-02 controller sketch still forwards the caller's key to Stripe, contradicting the section above it

- **What:** PPW-29's edit corrected the integration section but left the code sketch further down forwarding
  the caller's key, so the document contradicted itself. The shipped code was always right.
- **History:**
  - v6: found — raised by a lens during the verification of round 5
...
### PPW-36 — No test touches the Postgres path: not the constraint match, not the filtered index, not the migration
...
- **History:**
  - v8: found — re-raises the round 5 decision that deliberately scoped PPW-26's event narrowly
...
### PPW-38 — On SQLite the key violation is recognised by a phrase in the error message, not a structured code
...
- **History:**
  - v8: found — five lenses reached it independently, the pass's strongest agreement
...
### PPW-39 — One global key index tells an attacker whether a guessed key is in use, and lets them reserve it first
...
- **History:**
  - v8: found — re-raises the round 1 decision to keep one global key index; shares a root with PPW-17
...
### PPW-40 — The legacy processor recovery replay builds a fresh redirect URL instead of returning the stored one

- **What:** Stripe is deduplicated by the gateway because the call carries the order id as its key.
  the legacy processor has no such key, and the recovery path rebuilds the URL with a fresh timestamp, so two
...
- **Suggested fix:** Re-read the stored URL inside the recovery scope and reuse it when present, or take a
  short row lock. The lock needs the Postgres path that PPW-36 covers.
- **History:**
  - v8: found — three lenses reached it independently
  ...
  - post-close: fixed @`06fd2b1` — the replay path was deleted with the legacy processor (PR #13, recorded at 038-039 v13), so the defect has no code left; noted here because a backlog row leaves the queue only after its home row records the state
...
### PPW-42 — Freeing the stale key and inserting the new order are two separate saves with nothing to roll them back together
...
- **History:**
  - v8: found — re-raises the round 5 ruling that treated the same behaviour as a document error, PPW-28
...
### PPW-43 — On SQLite two racing requests can collide on the order-number index first, which the recovery does not handle
...
- **History:**
  - v4: raised as an observation while verifying PPW-18 and dismissed as not a finding
  - v8: found — the same behaviour, this time recorded as a defect, overturning that dismissal
...
### PPW-44 — The order service's price-tier lookup is a comment-claimed copy of the cart service's, but reads a different quantity
...
- **History:**
  - v8: found — the difference itself predates this work
...
### PPW-45 — The public key lookup has no production caller left
...
### PPW-46 — The cart seed graph is rebuilt in three test fixtures and has already drifted apart
...
### PPW-47 — The concurrency test hand-builds the winning order with copied number totals
...
### PPW-48 — The two replay-logging branches repeat the event shape and test the replay flag twice
...
### PPW-49 — The exception middleware reads the hosting environment two different ways in one request
...
### PPW-50 — The declared 409 response has no body type, so generated clients never see the divergent-field list
...
### PPW-51 — The transitional missing-key event logs at warning level on every payment request
