...
| Target | State |
|---|---|
| 038-039 invoicing | Open, v2 verification 2026-08-13 at `7cde815`: 21 of 24 fixes held, 2 reopened (PPW-481's replacement docstring is false in a new way; PPW-483's test stays green when the defect is reintroduced), and PPW-493 stays `fixed` because its Postgres test can only run in CI. <br> 3 new fix-caused findings: 🟠 PPW-506 (the rollout runbook still tells an operator that flipping the flag delivers invoices to customers), 🟡 PPW-507, 🟡 PPW-508. Two approach-check steps recorded as done are absent from the code (PPW-471's migration pre-flight, PPW-477's sanitize net). <br> `tests-coverage`/`db-parity` are still owed manifest lenses (PPW-497 — two requested lens keys were typos and silently dropped). <br> Next: a fix round — two reopened fixes re-arm the loop. |
