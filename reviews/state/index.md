...
| Target | State |
|---|---|
| 038-039 invoicing | Open, v3 fix round resolved 2026-08-14 at `0a250b9`: PPW-508's proofs now redden when the defect returns, closing the only row the v3 verification reopened. <br> Two 🟡 doc rows sit backlogged, PPW-509 and PPW-510. PPW-493 still stands `fixed`, never `verified` — its Postgres tests only execute in CI. Two approach-check steps recorded as done in round 1 are still absent from the code (PPW-471's migration pre-flight, PPW-477's sanitize net). <br> `tests-coverage`/`db-parity` are still owed manifest lenses (PPW-497 — two requested lens keys were typos and silently dropped). <br> Next: a verification pass against `0a250b9`; the round was patch-grade, so a clean one leaves the loop quiet. |
