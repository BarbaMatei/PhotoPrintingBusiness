...
| Target | State |
|---|---|
| 038-039 invoicing | Open, v5 verification 2026-08-20 at `07b0c1b`: PPW-508 verified and closed after four rounds — all three legs redden on their own revert, and the cancellation proof holds for the claimed reason, with the exception raised inside `ReloadAsync`. <br> New 🟡 PPW-511 — the EuPlatesc removal that two coverage waivers rest on is tracked nowhere, while `definition-of-done.md` class 2 still mandates the coverage; the live EuPlatesc call site stays proven by nothing, on the owner ruling. <br> Two 🟡 doc rows sit backlogged, PPW-509 and PPW-510. PPW-493 still stands `fixed`, never `verified` — its Postgres tests only execute in CI. Two approach-check steps recorded as done in round 1 are still absent from the code (PPW-471’s migration pre-flight, PPW-477’s sanitize net). <br> `tests-coverage`/`db-parity` are still owed manifest lenses (PPW-497 — two requested lens keys were typos and silently dropped). <br> Next: the loop is quiet — nothing reopened and the v4 round was test-only, so no delta discovery is owed. Certification waits for an explicit owner go-ahead. |
