...
| Target | State |
|---|---|
| 038-039 invoicing | Open, v4 fix round resolved 2026-08-14 at `07b0c1b`: all three of PPW-508's legs now redden on revert, the cancellation one included. <br> The owner has ruled EuPlatesc is being removed in favour of Stripe alone, so that call site's missing proof is dropped rather than written; the removal itself is not yet done and is not tracked on this ledger. <br> Two 🟡 doc rows sit backlogged, PPW-509 and PPW-510. PPW-493 still stands `fixed`, never `verified` — its Postgres tests only execute in CI. Two approach-check steps recorded as done in round 1 are still absent from the code (PPW-471's migration pre-flight, PPW-477's sanitize net). <br> `tests-coverage`/`db-parity` are still owed manifest lenses (PPW-497 — two requested lens keys were typos and silently dropped). <br> Next: a fix round — a reopened fix re-arms the loop. Scope is one test file: add the cancellation proof and cover the EuPlatesc call site. |
