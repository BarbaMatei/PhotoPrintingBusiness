...
| Target | State |
|---|---|
| 038-039 invoicing | Open, v3 verification 2026-08-14 at `08e7746`: 4 of the 5 v2 fixes verified, PPW-508 reopened because two of its three legs stay green when the defect is reintroduced. <br> Two new 🟡 doc rows, PPW-509 and PPW-510. PPW-493 still stands `fixed`, never `verified` — its Postgres tests only execute in CI. Two approach-check steps recorded as done in round 1 are still absent from the code (PPW-471's migration pre-flight, PPW-477's sanitize net). <br> `tests-coverage`/`db-parity` are still owed manifest lenses (PPW-497 — two requested lens keys were typos and silently dropped). <br> Next: a fix round — a reopened fix re-arms the loop. |
