...
| review system (meta) | Open, never archived: v1 on 2026-07-29 (14 standing, 13 fixed), v2 on 2026-08-12 (21 findings) with its fix round resolved the same day at `5245b81`. <br> 18 fixed incl. the reopened SF16 and all six dead or mis-wired checks; SF14 (seeded run 2) and SF24 stay deferred; SF31 wont-fix by owner ruling. <br> Next: an independent verification pass. Ids here are the `SF<n>` of the system reviews ([ledger](../system/ledger.md)), outside the `PPW-<n>` sequence, because `reviews/system/` is out of the doc contracts' scope. |
