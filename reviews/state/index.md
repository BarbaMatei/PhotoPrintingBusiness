...
| Target | State |
|---|---|
| 038-039 invoicing | Open, v2 fix round resolved 2026-08-14 at `f366c8a`: all 5 findings the v2 verification left open are fixed, including both reopened rows. <br> PPW-493 still stands `fixed`, never `verified` — its Postgres tests only execute in CI, so no local run has proven them. Two approach-check steps recorded as done in round 1 are still absent from the code (PPW-471's migration pre-flight, PPW-477's sanitize net). <br> `tests-coverage`/`db-parity` are still owed manifest lenses (PPW-497 — two requested lens keys were typos and silently dropped). <br> Next: a verification pass against `f366c8a`; the round was patch-grade, so a clean one leaves the loop quiet. |
