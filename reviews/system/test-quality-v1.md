...
| TQ9 | `CancelOrderAsync_StripeOrder_AttemptsStripeRefund` never verifies the refund; the Stripe branch is deletable — a customer's money silently not returned ([AdminOrderServiceTests.cs:663](../../src/PhotoPrint.Tests/Unit/Services/AdminOrderServiceTests.cs#L663)) | 🔴→🟠: single branch; the legacy processor sibling does it right (`Times.Once`) |
