...
findings:
  ...
  SF5:  { status: open,           commit: null,    note: "schema v2 + records-auditor script — build item 2 in review-v1 recommendations" }
  SF6:  { status: open,           commit: null,    note: "fix_generated count — part of schema v2" }
  SF7:  { status: open,           commit: null,    note: "label roll-ups 'pass costs only' (done in review-v1's own table); coarse per-round cost line = owner's call at schema v2" }
  ...
  SF9:  { status: open,           commit: null,    note: "comment-citation sweep (371 hits / 118 files) + blinding auditor — build items 2–3" }
  ...
  SF12: { status: open,           commit: null,    note: "records auditor covers it — build item 2" }
...
| SF5 | 🟡 | open | schema v2 + records auditor |
| SF6 | 🟡 | open | schema v2 (`fix_generated`) |
| SF9 | 🟡 | open | citation sweep + blinding auditor |
| SF15 | 🟡 | fixed | R4 in runbook-verification step 1; 015 ledger expiry resolved |
| SF7 | ⚪ | open | labeling done in review-v1; per-round cost line = owner call |
| SF8 | ⚪ | fixed | three stale quotes corrected |
| SF12 | ⚪ | open | records auditor |
...
Remaining build order (owner-sequenced, from review-v1 recommendations): records auditor →
comment-citation sweep → post-cert-escape marker → schema v2 → loop-driver skill. Parked:
seeded run 2 (SF14). Verification of this round's fixes = the future re-grade
