...
| Latest review has open 🔴/🟠 findings and no `resolved` resolution answering it | **Fix round** (`/fix-review`) — new 🟡/⚪ go to the ledger **backlog**, not the fix round (stop rule below) |
| Latest resolution is `resolved` but not yet re-reviewed | **Verification** pass |
| Verification clean (0 reopened), fix round was **delta-worthy** (fixed a 🔴, added/converted a mechanism, or changed a design), and no delta since | **Delta discovery** |
| Verification clean (0 reopened), fix round patch-grade (none of the above) | Nothing — the loop is **quiet** (stop rule below) |
| Loop quiet | **Certification** (risk-tiered, stop rule below): freeze the commit, two parallel blinded full passes — folding in any still-owed manifest lenses |
| Certification pair quiet | **Certified** — the only path to `approved`; loop done |
| Any pass raised a new 🔴, or a fix-caused 🟠 **regression** | Back to **Fix round** (the counter resets) |
...
### The stop rule (severity-based — 2026-07-22, from the 043 cost data)

043 exposed the old exit ("delta quiet = no new 🔴/🟠 of any kind") as effectively unreachable: a
detector this sensitive finds *something* in any non-trivial diff, every fix round re-armed a
delta, and the two deltas cost 2.96M tokens to find 0 Highs — nearly every finding generated by
our own fixes. The loop now keys on severity, which was the design intent all along
([self-driving-loop-design.md](self-driving-loop-design.md), *When the loop should stop*):

- **The loop stays open for exactly three things:** a new 🔴 anywhere; a new 🟠 that is a
  **fix-caused regression** (a fix broke behavior that previously worked); a reopened fix. Each
  sends the router back to a fix round and resets the quiet counter.
- **New non-regression 🟠 findings** get fixed in the next fix round and verified — but they do
  not by themselves re-arm a delta. The check on that fix round is verification plus the fixer's
  fresh-eyes micro-review, unless the round was **delta-worthy**: it fixed a 🔴, added/converted
  a mechanism, or changed a design (the same trigger list as fixer rule #3).
- **New 🟡/⚪ findings enter the ledger with status `backlog`.** They never re-arm the loop and
  never gate closure. They are drained deliberately, not dropped: batched into a later sweep (the
  design doc's *backlog groomer*), picked up by the next bolt touching that area, or re-judged
  fresh by the blinded certification pair.
- **Certification is risk-tiered** per the entry policy in
  [self-driving-loop-design.md](self-driving-loop-design.md): targets in the full-loop tier
  (money, auth, data loss, concurrency, migrations, new external input — 043's storage qualifies)
  get the two-parallel-pass certification, **folding any still-owed manifest lenses into the
...
| Lenses | The lenses owning the fix classes + `correctness` + `completeness-critic` (~4–5 — the script hard-caps a delta pass at 5 lenses; both 043 deltas ran 6–7 and blew the budget 2–3×) |
| Cost | ~400–600k tokens — vs ~2M for a full pass. **Enforced:** the script defaults `tokenBudget` to 600k output tokens on delta passes and skips remaining skeptics past it (verdict `unverified-over-budget`, logged) |
...
After every fix round: verification (unchanged). Delta discovery only after a **delta-worthy**
fix round — one that fixed a 🔴, added/converted a mechanism, or changed a design; a patch-grade
fix round exits on verification + the fixer's micro-review (see *The stop rule* — 043's two
deltas over fix rounds cost 2.96M tokens and found 0 Highs). Repeat until the loop is quiet,
then freeze and certify. What a delta pass structurally cannot see —
...
- **#2 Convergence-weighted adversarial verify:** ≥3 lenses agree and not `hinted` → one
  ...
  `unverified-cleanup` for ⚪ (skeptics skipped). On `passType: 'delta'` the tiers cut deeper —
  ≥3 non-hinted → `confirmed` with no skeptic; 🔴/🟠 → one trace (guard only if no trace); 🟡/⚪
  → none (`unverified-low`) — and a `tokenBudget` (default 600k on delta) skips remaining
  skeptics once exceeded (`unverified-over-budget`). See *Tiering the adversarial skeptics*.
...
"plausible". ⚪ cleanups skip skeptics entirely and carry `unverified-cleanup`; on delta passes
🟡 lows do too (`unverified-low`), and findings past the pass's `tokenBudget` carry
`unverified-over-budget` — all three mean "lens verdict unchallenged", not "refuted". Security findings additionally carry a
...
- **Delta-discovery passes** sit between the two (see *The middle tier*): blinded manifest-subset
  lenses (≤5, script-enforced) over the cumulative fix diff, ~400–600k tokens (script-enforced via
  `tokenBudget`), after **delta-worthy** fix rounds only — so full passes run only at the ends
...
**Delta passes cut deeper (2026-07-22).** The 043 calibration: ~75 skeptics across its three
discovery passes (v1/v3/v5) refuted **2** findings — ~3% yield on a surface where lens precision
was already high. On a delta pass the script therefore runs: convergence ≥3 non-hinted →
`confirmed`, **no skeptic** (agreement is the precision signal); 🔴/🟠 → **one** trace-constructor,
escalating to a guard-hunt only if no trace builds; 🟡 → **none** (verdict `unverified-low` — the
lens verdict stands and the fixer judges); ⚪ → none. On 043-v5's mix that is ~3 skeptics instead
of 15. Full discovery/certification passes keep the table above — they are the safety net.
