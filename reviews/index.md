...
ledger, never here; lessons and cross-target numbers in [rationale.md](rationale.md). Pass
...
| review system (meta) | Open, never archived: one pass, v1 on 2026-07-29, 16 findings raised and 14 standing (0🔴/5🟠/5🟡/4⚪). <br> Worst is SF14 — two targets were certified under a stop rule whose gating experiment never ran; SF1 records 0 completions of the undeviated certification path. <br> Recommendations 1–7 await the owner's decision. Ids here are the `SF<n>` of `system/review-v1.md`, outside the `PPW-<n>` sequence, because `reviews/system/` is out of the doc contracts' scope. |
...
| 2026-06-19 | 035 | v1–v7 (pre-system loop) | approved (v7 — verdict later shown premature) | 15 (v1) | Pre-system loop: 15 findings at v1 — 12 verified, 2 wont-fix, 1 backlogged. This is the loop that taught the two-loops split ([rationale](rationale.md)) | [review] · [resolution](archive/035-payment-idempotency/resolution-v5.md) |
