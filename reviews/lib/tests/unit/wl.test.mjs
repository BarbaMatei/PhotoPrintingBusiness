...
// ---------- wl: the validated worklog stamper ----------
{
  ...
  let r = run('records/wl.mjs', ['--root', T, target, 'pass-launch', '--pass', 'v1', '--type', 'full discovery'])
  ...
  r = run('records/wl.mjs', ['--root', T, target, 'not-a-real-event'])
  ...
  r = run('records/wl.mjs', ['--root', T, target, 'finding', '--id', 'BUG-1', '--status', 'fixed'])
  check('wl refuses a finding id not shaped like PPW-<n>', r.code === 1 && r.out.includes('ERROR'), r.out.trim())

  r = run('records/wl.mjs', ['--root', T, target, 'run-start', '--t', '2020-01-01T00:00:00+00:00'])
  ...
  r = run('records/wl.mjs', ['--root', T, target, 'round-start', '--round', '7'])
  ...
  r = run('records/wl.mjs', ['--root', T, target, 'void', '--json', '{"of":{"ev":"round-start","t":"1999-01-01T00:00:00+00:00"}}'])
  ...
  if (firstEvent) {
    r = run('records/wl.mjs', ['--root', T, target, 'void', '--json', JSON.stringify({ of: { ev: firstEvent.ev, t: firstEvent.t } })])
...
}
{
  ...
  let r = run('records/wl.mjs', ['--root', T, target, 'round-start', '--round', '1'])
  ...
  if (opened) {
    r = run('records/wl.mjs', ['--root', T, target, 'void', '--json', JSON.stringify({ of: { ev: 'round-start', t: opened.t, round: 1 } })])
...
}
{
  ...
  const r = run('records/wl.mjs', ['--root', T, target, 'note', '--text', 'archived target event'])
...
}
{
  ...
  let r = run('records/wl.mjs', ['--root', T, target, 'protocol-written', '--round', '1', '--cluster', 'c1', '--ids', 'PPW-1,PPW-2'])
  ...
  r = run('records/wl.mjs', ['--root', T, target, 'check-dispatched', '--round', '1', '--cluster', 'c1', '--json', '{"ids":["PPW-3"]}'])
  check('wl accepts check-dispatched with round/cluster/ids', r.code === 0, r.out.trim())

  r = run('records/wl.mjs', ['--root', T, target, 'check-returned', '--round', '1', '--cluster', 'c1', '--verdict', 'cleared'])
  check('wl accepts check-returned with round/cluster/verdict', r.code === 0, r.out.trim())

  r = run('records/wl.mjs', ['--root', T, target, 'check-returned', '--round', '1', '--cluster', 'c1', '--verdict', 'cleared', '--tokens', '1200'])
  check('wl still accepts check-returned with the optional tokens field', r.code === 0, r.out.trim())

  r = run('records/wl.mjs', ['--root', T, target, 'round-review-dispatched', '--round', '1'])
  check('wl accepts round-review-dispatched with round', r.code === 0, r.out.trim())

  r = run('records/wl.mjs', ['--root', T, target, 'round-review-returned', '--round', '1', '--found', '0'])
  check('wl accepts round-review-returned with round/found', r.code === 0, r.out.trim())

  r = run('records/wl.mjs', ['--root', T, target, 'test-audit-dispatched', '--round', '1'])
  check('wl accepts test-audit-dispatched with round', r.code === 0, r.out.trim())

  r = run('records/wl.mjs', ['--root', T, target, 'test-audit-returned', '--round', '1', '--verdict', 'pass'])
  ...
  r = run('records/wl.mjs', ['--root', T, target, 'protocol-written', '--round', '1', '--cluster', 'c1', '--ids', 'BUG-1'])
  check('wl refuses ids not shaped like PPW-<n>', r.code === 1 && r.out.includes('ERROR'), r.out.trim())

  r = run('records/wl.mjs', ['--root', T, target, 'protocol-written', '--round', '1', '--cluster', 'c1', '--json', '{"ids":[]}'])
  check('wl refuses an empty ids array', r.code === 1 && r.out.includes('ERROR'), r.out.trim())

  r = run('records/wl.mjs', ['--root', T, target, 'check-dispatched', '--round', '1', '--cluster', 'c1', '--json', '{"ids":"PPW-1"}'])
  check('wl refuses ids that is a string instead of an array', r.code === 1 && r.out.includes('ERROR'), r.out.trim())

  r = run('records/wl.mjs', ['--root', T, target, 'protocol-written', '--round', '1', '--cluster', 'c1', '--ids', 'PPW-1, PPW-2'])
  ...
  r = run('records/wl.mjs', ['--root', T, target, 'protocol-written', '--round', '1', '--cluster', 'c1', '--ids', 'PPW-1,BUG-2'])
  check('wl still refuses a non-PPW element inside a comma-separated --ids list', r.code === 1 && r.out.includes('ERROR'), r.out.trim())

  r = run('records/wl.mjs', ['--root', T, target, 'protocol-written', '--cluster', 'c1', '--ids', 'PPW-1', '--json', '{"round":"one"}'])
